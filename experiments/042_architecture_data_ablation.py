"""Isolate experiment 041's architecture and data mix from experiment 026.

Both treatments use 026's model size, BC objective, optimizer, schedule,
training budget, and base observations. The ``arch-only`` treatment trains on
ranked-anonymized-1. The ``full-mix`` treatment changes only the corpus to all
six ranked-anonymous archives and all professional archives. Every stream has
``repeat=1``; Mosaic therefore samples the full mix by natural stream length.

The copied 041 architecture keeps SwiGLU projections, normalized linear action
heads without the trunk-logit skip, normalized temporal input, symmetric
residual scaling, and bounded identity-initialized group conditioning. It also
keeps 041's resumable one-window-per-replay sampler and prefetch pipeline.

Run:
    uv run experiments/042_architecture_data_ablation.py train --cfg.treatment arch-only
    uv run experiments/042_architecture_data_ablation.py train --cfg.treatment full-mix
    uv run experiments/042_architecture_data_ablation.py benchmark-train-step
    uv run experiments/042_architecture_data_ablation.py eval --checkpoint runs/<run>/final.pt
"""
...
_EXPERIMENT_ID = "042_architecture_data_ablation_v1"
_INFERENCE_BUCKETS = (1, 2, 4, 8, 16, 32, 64)
_PRODUCTION_LOSS_POSITIONS = 2**30
...
_FULL_MIX_SOURCE_NAMES = tuple(source.name for source in streams.POLICY_WORLD_V7_SOURCES)
_ARCH_ONLY_SOURCE_NAMES = (streams.RANKED_ANONYMIZED_1_POLICY_V7.name,)
...
class TrainConfig:
    ...
    treatment: Literal["arch-only", "full-mix"] = "arch-only"
    ...
    final_diagnostic_n_matchups: int = 32
    ...
    def source_names(self) -> tuple[str, ...]:
        """Return the immutable source set for this treatment."""
        if self.treatment == "arch-only":
            return _ARCH_ONLY_SOURCE_NAMES
        if self.treatment == "full-mix":
            return _FULL_MIX_SOURCE_NAMES
        raise ValueError(f"unknown treatment: {self.treatment!r}")

    @property
    def replay_format(self) -> Literal["policy", "policy-world"]:
        """Return the on-disk replay format for this treatment."""
        return "policy" if self.treatment == "arch-only" else "policy-world"

    @property
    def cache_limit_gb(self) -> int:
        """Reserve enough local MDS cache for the selected corpus."""
        return 160 if self.treatment == "arch-only" else 1792
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if offsets[:_N_PRIMARY] != tuple(range(1, _N_PRIMARY + 1)):
        raise ValueError("the primary bucket must be the dense offset prefix 1..4")
    ...
    if cfg.treatment not in ("arch-only", "full-mix"):
        raise ValueError("treatment must be 'arch-only' or 'full-mix'")
    if cfg.treatment == "full-mix" and not set(cfg.source_names) <= _POLICY_WORLD_NAMES:
        raise ValueError("full-mix sources must use the policy-world-v7 corpus")
    for name, value in (
    ...
    if not 0 <= cfg.warmup_steps < cfg.max_steps:
...
def validate_production_config(cfg: TrainConfig) -> None:
    """Require frozen scientific settings for the selected treatment."""
    expected = asdict(TrainConfig(treatment=cfg.treatment))
    actual = asdict(cfg)
...
def synthetic_context(cfg: TrainConfig, batch_size: int, device: torch.device) -> Context:
    """Build a synthetic base observation batch."""
...
def prepared_targets(
    ...
    """Quantize history and future once, then align every selected offset."""
    ...
    targets = torch.stack(
        [full[:, offset : offset + length] for offset in model.head_offsets],
        dim=2,
    )
...
class DeviceBatchPrefetcher:
    """Fetch the next CPU batch during GPU compute and stage its transfer."""
    ...
    def __init__(
        ...
        self._pool = ThreadPoolExecutor(
            max_workers=1,
            thread_name_prefix="device-batch-prefetch",
        )
        self._future: Future[TrainBatch] | None = None
        self.preload()
    ...
    def _load_cpu_batch(self) -> TrainBatch:
        ...
        if not isinstance(cpu_batch, TrainBatch):
            raise TypeError(f"loader yielded {type(cpu_batch).__name__}, expected TrainBatch")
    ...
    def preload(self) -> None:
        ...
        self._stage(self._load_cpu_batch())
    ...
    def finish_preload(self) -> float:
        """Stage a background-loaded batch and return its uncovered wait."""
        if self._future is None:
            raise RuntimeError("no background preload is running")
        started = time.monotonic()
        future, self._future = self._future, None
        self._stage(future.result())
...
def temporal_objective_parts(
    ...
    *,
    ...
    """Return 026's dense-four primary loss plus mean auxiliary loss."""
    joint_nll = nll.float().sum(dim=-1)
    joint_nll = torch.where(valid[..., None], joint_nll, 0)
    primary = joint_nll[..., :_N_PRIMARY].sum() / (valid_prefixes * _N_PRIMARY)
    auxiliary_count = nll.shape[-2] - _N_PRIMARY
    auxiliary = joint_nll[..., _N_PRIMARY:].sum() / (valid_prefixes * auxiliary_count)
    return primary, auxiliary, primary + auxiliary
...
def microbatch_loss(
    ...
    *,
    ...
    """Compute the exact experiment 026 behavior-cloning objective."""
    del step
    ...
    with amp_context(cfg, DEVICE):
        ...
        primary, auxiliary, loss = temporal_objective_parts(
            dense_nll,
    ...
    extra = {
        ...
        "aux/primary_loss": primary.detach() / _LN2,
        "aux/auxiliary_loss": auxiliary.detach() / _LN2,
        **button_diagnostics,
    }
    if phase_timer is not None:
...
def nll_mean_metrics(
    ...
    if len(offsets) <= _N_PRIMARY:
        raise ValueError(f"the {_N_PRIMARY} primary offsets must leave an auxiliary offset")
    primary = joint[:_N_PRIMARY].mean()
    auxiliary = joint[_N_PRIMARY:].mean()
    out = {
        "loss": float(primary + auxiliary),
        "primary_nll": float(primary),
        "auxiliary_nll": float(auxiliary),
    }
    for depth, offset in enumerate(offsets):
...
def _validation_wandb_metrics(values: dict[str, float], cfg: TrainConfig) -> dict[str, float]:
    ...
    return {
        "nll": values["loss"],
        "primary_nll": values["primary_nll"],
        "auxiliary_nll": values["auxiliary_nll"],
...
class BF16Inference:
    ...
    def decode(
        ...
        *,
        ...
        rows = ctx.ctx_pad.shape[0]
        bucket = self._bucket(rows)
        padded = canonical_context(_pad_context(ctx, bucket), "base", items=False)
    ...
    *,
    ...
    def predict(ctx: Context, committed: np.ndarray | None) -> np.ndarray:
        if committed is not None:
            raise ValueError("experiment 042 does not condition on a committed RTC prefix")
        started = time.perf_counter()
...
def approximate_training_flops_per_update(cfg: TrainConfig, parameter_counts: dict[str, int]) -> int:
    ...
    shared_parameters = parameter_counts["trunk"] + parameter_counts["other"]
...
def model_tag(cfg: TrainConfig) -> str:
    ...
    return (
        f"ablate042-d{cfg.arch.d_model}-L{cfg.arch.n_layers}-h{cfg.arch.n_heads}-Lc{cfg.arch.L_ctx}-"
        f"t{cfg.arch.temporal_d_model}x{cfg.arch.temporal_layers}-o{offsets}-s{cfg.exec_horizon}-"
        f"linear-head-no-skip-bc-{cfg.treatment}"
    )
...
def _diagnostic_batch(batch: TrainBatch, max_rows: int) -> TrainBatch:
    """Take a small device-resident prefix for the occasional eager diagnostic."""
    if max_rows < 1:
        raise ValueError(f"max_rows must be positive, got {max_rows}")
    rows = min(max_rows, batch.target.shape[0])
...
def source_mixture_weights(cfg: TrainConfig) -> tuple[float, ...]:
    """Return source weights used only to merge feature statistics."""
    if cfg.treatment == "arch-only":
        return (1.0,)
...
def _make_loaders(cfg: TrainConfig, stats: dict[str, FeatureStats]):
    ...
    validation: _LoaderKwargs = {
        **common,
        "batch_size": cfg.val_batch_size,
        "sources": (streams.RANKED_ANONYMIZED_1_POLICY_V7,),
    }
...
def _init_wandb(cfg: TrainConfig, run_name: str, resume_state: dict | None) -> None:
    ...
    wandb.run.summary["nll_semantics"] = (
        "026 BC objective: mean joint NLL at offsets 1-4 plus mean joint NLL at offsets 5,6,9,12,16,20"
    )
    ...
    wandb.run.summary["architecture/treatment"] = (
        ...
        "residuals; bounded identity-initialized group conditioning"
    )
    wandb.run.summary["data/sampler"] = "041-resumable-one-window-per-replay"
    wandb.run.summary["data/source_mixing"] = "repeat_1-natural-length"
...
def _finalize_training(
    *,
    ...
    if final_matchups:
        ...
        if not smoke and cfg.final_diagnostic_n_matchups:
            diagnostic_eval = eval_vs_cpu(
                model,
                stats,
                cfg,
                n_matchups=cfg.final_diagnostic_n_matchups,
                replay_dir=replay_dir / "final-s6",
                exec_horizon=6,
                checkpoint_sha256=checkpoint_sha,
                inference=eval_inference,
            )
            require_complete_eval(
                diagnostic_eval,
                cfg.final_diagnostic_n_matchups,
            )
            final_metrics.update(
                {f"eval_s6/{name}": value for name, value in _eval_wandb_metrics(diagnostic_eval).items()}
            )
...
def train(
    ...
    *,
    ...
    data_tag = "ranked-anon-1-policy-v7" if cfg.treatment == "arch-only" else "policy-world-v7-all-repeat1"
...
_RUNTIME_CONFIG_FIELDS = frozenset(item.name for item in fields(TrainConfig)) - {"arch"}
...
def config_from_state(values: dict) -> TrainConfig:
    ...
    expected = {"experiment_id", "architecture", *_RUNTIME_CONFIG_FIELDS}
    ...
    return TrainConfig(arch=architecture, **runtime)
