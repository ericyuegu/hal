"""011a: 009 + torch.compile + eval-cadence control (throughput speedrun base).

Identical model/objective to 009; the only changes are systems-level:
  * ``torch.compile`` the training forward (eval/decode use the eager model);
  * jaxtyped/beartype runtime decorators stripped from the hot path (graph breaks);
  * eval/val cadence fully gated on ``eval_every``/``val_every`` (incl. the final
    pass) so throughput runs do zero closed-loop eval;
  * ``push_to_r2`` defaults False (local speedrun).
...
def eval_temp_sweep(ckpt_path: str, temps: tuple[float, ...], *, replicas: int, max_frames: int) -> None:
    """Lean test-time decode-temperature sweep on FD vs lvl-9 CPU (no retraining). One stage,
    configurable replicas/frames — a cheap screen for the best deploy temperature. Prints pooled
    vs-cpu metrics + stock winrate per temp."""
    model, cfg, stats, state = _load_ckpt(ckpt_path)
...
class Args:
    ...
    sweep_temps: str = "0.7,0.85,1.0"  # comma-sep decode temps for --eval-temp-sweep
