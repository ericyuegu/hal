"""Experiment 057: scaled categorical-endpoint flow.

This standalone experiment combines O38's endpoint objective with O52's
ranked-1 data path, observation representation, history trunk, initialization,
and run infrastructure. The registered study runs proxy AdamW, proxy Muon, and
production Muon in that order. AWR is implemented but disabled in those arms.
...
Run:
    uv run experiments/057_scaled_endpoint_flow.py train --arm proxy_adamw
    uv run experiments/057_scaled_endpoint_flow.py train --arm proxy_muon
    uv run experiments/057_scaled_endpoint_flow.py train --arm production_muon
    uv run experiments/057_scaled_endpoint_flow.py eval --checkpoint runs/<run>/final.pt
"""
...
_EXPERIMENT_ID: Final[str] = "057_scaled_endpoint_flow_v1"
_STARTUP_LOG_INTERVAL_S: Final[float] = 60.0
HISTORICAL_REFERENCE_RUN: Final[str] = "0ksityvv"
HISTORICAL_REFERENCE_UPDATE: Final[int] = 16_384
HISTORICAL_REFERENCE_EXPERIMENT_ID: Final[str] = "052_adamw_temporal_awr_v1"
HISTORICAL_REFERENCE_SOURCE_SHA256: Final[str] = "d0e75291dc3e0c322aea0cdd8b57573c5fec20b2c843b71943b8e3777f247b69"
...
class Architecture:
    ...
    flow_reference_layers: ClassVar[int] = 2
    trunk_reference_attention_scale: ClassVar[float] = 0.25
    flow_reference_attention_scale: ClassVar[float] = 0.5
...
class TrainConfig:
    ...
    materialization_threads_env: ClassVar[str] = "HAL_O57_MATERIALIZATION_THREADS"
    data_protocol: ClassVar[str] = "o57-rank1-replay-ring-v1"
    ...
    solver: Literal["uniform4", "uniform8", "historical4"] = "uniform4"

    source_names: tuple[str, ...] = (streams.RANKED_ANONYMIZED_POLICY_WORLD_V8[0].name,)
    ...
    def exec_horizon(self) -> int:
        """Compatibility name used by the generic zero-delay H2H loader."""
        return self.replan_interval_frames
...
def validate_config(cfg: TrainConfig) -> None:
    positive = {
        ...
        "flow_d_model": cfg.arch.flow_d_model,
        "flow_layers": cfg.arch.flow_layers,
        "flow_heads": cfg.arch.flow_heads,
        "flow_ff_dim": cfg.arch.flow_ff_dim,
    ...
    if cfg.arch.d_model % cfg.arch.n_heads or cfg.arch.flow_d_model % cfg.arch.flow_heads:
        raise ValueError("model dimensions must be divisible by their head counts")
    if cfg.arch.flow_d_model // cfg.arch.flow_heads != 64:
        raise ValueError("flow attention fixes the head dimension at 64")
    if cfg.arch.flow_d_model % CONTROLLER_GROUP_COUNT:
        raise ValueError("flow width must divide evenly across controller groups")
    ...
    if cfg.prediction_frames != len(offsets):
        raise ValueError("prediction_frames must solve all ten selected offsets")
    _validate_deployment_timing(cfg.prediction_frames, cfg.delay_frames, cfg.replan_interval_frames)
    if cfg.delay_frames > 4 or cfg.replan_interval_frames not in (1, 2):
        raise ValueError("flow runtime supports delay 0..4 and replanning every one or two frames")
    ...
    if cfg.source_names != (streams.RANKED_ANONYMIZED_POLICY_WORLD_V8[0].name,):
        raise ValueError("O57 requires only ranked-anonymized-1-policy-world-v8")
    if cfg.depth_alpha != 0.5 or cfg.hidden_std_multiplier != 0.5 or cfg.readout_init != "mup-normal":
        raise ValueError("O57 uses O50's selected depth and initialization parameterization")
    if (cfg.adam_beta1, cfg.adam_beta2, cfg.adam_eps) != (*cfg.base_adam_betas, cfg.base_adam_eps):
        raise ValueError("O57 scales the fixed base Adam betas and epsilon")
    ...
    if cfg.solver not in ("uniform4", "uniform8", "historical4"):
        raise ValueError("unknown flow solver")
    if cfg.execution_batch_size < 1 or cfg.batch_size % cfg.execution_batch_size:
        raise ValueError("execution_batch_size must divide the logical batch")
    if cfg.max_steps != 16_384 or cfg.warmup_steps != 512:
        raise ValueError("O57 fixes training to 16,384 updates with 512 warmup updates")
    if cfg.arch.parameter_count_contract["total"] not in (15_217_066, 228_682_954):
        raise ValueError("O57 supports only the registered proxy and production architectures")
    fixed_training = {
        "batch_size": (cfg.batch_size, 512),
        "seed": (cfg.seed, 0),
        "eval_seed": (cfg.eval_seed, 0),
        "adam_lr": (cfg.adam_lr, 1.7e-3),
        "muon_lr": (cfg.muon_lr, 0.014),
        "adam_weight_decay": (cfg.adam_weight_decay, 1e-4),
        "muon_weight_decay": (cfg.muon_weight_decay, 1e-4),
        "grad_clip": (cfg.grad_clip, 1.0),
        "lr_floor_ratio": (cfg.lr_floor_ratio, 1 / 170),
        "amp_dtype": (cfg.amp_dtype, "bfloat16"),
        "allow_tf32": (cfg.allow_tf32, True),
        "identity_dropout": (cfg.identity_dropout, 0.10),
    }
    changed = {name: actual for name, (actual, expected) in fixed_training.items() if actual != expected}
    if changed:
        raise ValueError(f"O57 fixed training settings changed: {changed}")
...
type StudyArm = Literal["proxy_adamw", "proxy_muon", "production_muon"]
STUDY_ARM_ORDER: Final[tuple[StudyArm, ...]] = (
    "proxy_adamw",
    "proxy_muon",
    "production_muon",
)
...
def config_for_arm(arm: StudyArm) -> TrainConfig:
    """Return one registered treatment without changing corpus or duration."""
    if arm == "proxy_adamw":
        return proxy_config()
    if arm == "proxy_muon":
        return replace(proxy_config(), optimizer="muon")
    if arm == "production_muon":
        return replace(TrainConfig(), optimizer="muon")
    raise ValueError(f"unknown O57 study arm {arm!r}")


def study_arm_for_config(cfg: TrainConfig) -> StudyArm | None:
    """Return the registered arm matching a checkpoint's model and optimizer."""
    for arm in STUDY_ARM_ORDER:
        treatment = config_for_arm(arm)
        if (
            cfg.arch,
            cfg.optimizer,
            ...
            cfg.awr_enabled,
            cfg.delay_frames,
            cfg.replan_interval_frames,
            cfg.solver,
            cfg.automatic_evaluation,
        ) == (
            treatment.arch,
            treatment.optimizer,
            treatment.target_positions,
            treatment.awr_enabled,
            treatment.delay_frames,
            treatment.replan_interval_frames,
            treatment.solver,
            treatment.automatic_evaluation,
        ):
            return arm
    return None
...
def validate_historical_reference(
    ...
    *,
    ...
    """Reject a historical O52 reference whose checkpoint or source identity drifted."""
    if checkpoint_sha256(checkpoint) != expected_checkpoint_sha256:
        raise ValueError("historical O52 checkpoint SHA-256 mismatch")
    if hashlib.sha256(source.read_bytes()).hexdigest() != HISTORICAL_REFERENCE_SOURCE_SHA256:
        raise ValueError("historical O52 source SHA-256 mismatch")
    step = state.get("step")
    if not isinstance(step, int) or isinstance(step, bool) or step + 1 != HISTORICAL_REFERENCE_UPDATE:
        raise ValueError("historical O52 checkpoint is not update 16,384")
    config = state.get("cfg")
    if not isinstance(config, Mapping):
        raise ValueError("historical reference has no configuration")
    typed_config = cast(Mapping[str, object], config)
    if typed_config.get("experiment_id") != HISTORICAL_REFERENCE_EXPERIMENT_ID:
        raise ValueError("historical reference is not the registered O52 experiment")
...
_CUDA_PHASES: tuple[tuple[str, str, str], ...] = (
    ...
    ("flow", "trunk_end", "flow_end"),
    ("objective", "flow_end", "objective_end"),
...
def depth_rule(stack: Literal["trunk", "flow"], layers: int, alpha: float) -> DepthRule:
    """Return O51's residual multipliers for one stack."""
    if alpha != 0.5:
        raise ValueError("O57 fixes depth_alpha to 0.5")
    if layers < 1:
    ...
    if stack == "trunk":
        ...
        base_layers = Architecture.flow_reference_layers
        base_attention = Architecture.flow_reference_attention_scale
    else:
...
class FlowBlock(nn.Module):
    """Bidirectional flow block with AdaRMSNorm-Zero conditioning."""

    def __init__(self, cfg: TrainConfig) -> None:
        super().__init__()
        width = cfg.arch.flow_d_model
        self.n_heads = cfg.arch.flow_heads
        self.head_dim = width // self.n_heads
        self.width = width
        rule = depth_rule("flow", cfg.arch.flow_layers, cfg.depth_alpha)
        self.attention_scale = rule.attention
        self.ffn_scale = rule.mlp
        self.qkv = nn.Linear(width, 3 * width, bias=False)
        self.attention_output = nn.Linear(width, width, bias=False)
        self.ffn_up = nn.Linear(width, 2 * cfg.arch.flow_ff_dim, bias=False)
        self.ffn_down = nn.Linear(cfg.arch.flow_ff_dim, width, bias=False)
    ...
    def _modulate(x: Tensor, shift: Tensor, scale: Tensor) -> Tensor:
        return decoder_rmsnorm(x) * (1.0 + scale[:, None]) + shift[:, None]

    def forward(self, x: Tensor, condition: Tensor) -> Tensor:
        shift_a, scale_a, gate_a, shift_f, scale_f, gate_f = self.modulation(condition).chunk(6, dim=-1)
        attention_input = self._modulate(x, shift_a, scale_a)
        batch, length, _ = attention_input.shape
        q, k, v = self.qkv(attention_input).chunk(3, dim=-1)
        ...
        x = x + self.attention_scale * gate_a[:, None] * self.attention_output(attended)

        ffn_input = self._modulate(x, shift_f, scale_f)
        gate, value = self.ffn_up(ffn_input).chunk(2, dim=-1)
        ffn = self.ffn_down(F.silu(gate) * value)
        return x + self.ffn_scale * gate_f[:, None] * ffn
...
def sinusoidal_time_embedding(tau: Tensor, dim: int = 128) -> Tensor:
    """Embed scalar flow time without depending on model width."""
    if dim % 2:
        raise ValueError("flow-time embedding width must be even")
    frequencies = torch.exp(torch.linspace(0.0, -math.log(10_000.0), dim // 2, device=tau.device, dtype=torch.float32))
    angles = tau.float()[..., None] * frequencies * (2.0 * math.pi)
    return torch.cat((angles.sin(), angles.cos()), dim=-1)


class CategoricalEndpointFlow(nn.Module):
    """Categorical endpoint flow over ten future controller tokens."""

    def __init__(self, cfg: TrainConfig, codec: DiscreteControllerCodec) -> None:
        super().__init__()
        self.codec = codec
        self.head_offsets = tuple(cfg.arch.head_offsets)
        self.d_model = cfg.arch.flow_d_model
        quarter = self.d_model // CONTROLLER_GROUP_COUNT
        self.action_projections = nn.ModuleList(
            nn.Linear(vocab, quarter, bias=False) for vocab in CONTROLLER_GROUP_VOCABS
        )
        self.action_fusion_in = nn.Linear(self.d_model, 2 * self.d_model)
        self.action_fusion_out = nn.Linear(2 * self.d_model, self.d_model)
        self.offset_embedding = nn.Embedding(cfg.arch.sample_chunk_length + 1, self.d_model)
        self.committed_embedding = nn.Embedding(2, self.d_model)
        self.history_projection = nn.Linear(cfg.arch.d_model, self.d_model)
        self.time_in = nn.Linear(128, self.d_model)
        self.time_out = nn.Linear(self.d_model, self.d_model)
        self.condition_in = nn.Linear(2 * self.d_model, 2 * self.d_model)
        self.condition_out = nn.Linear(2 * self.d_model, self.d_model)
        self.blocks = nn.ModuleList([FlowBlock(cfg) for _ in range(cfg.arch.flow_layers)])
    ...
    def _condition(self, hidden: Tensor, tau: Tensor) -> Tensor:
        history = self.history_projection(decoder_rmsnorm(hidden))
        time = self.time_out(F.silu(self.time_in(sinusoidal_time_embedding(tau))))
        return self.condition_out(F.silu(self.condition_in(torch.cat((history, time), dim=-1))))
    ...
    def forward(
        ...
        tau: Tensor,
        committed: Tensor,
        trunk_logits: tuple[Tensor, ...] | None = None,
    ) -> dict[str, Tensor]:
        prefix = hidden.shape[:-1]
        offsets = len(self.head_offsets)
        expected_mask = (*prefix, offsets)
        if committed.shape != expected_mask:
            raise ValueError(f"committed mask must have shape {expected_mask}, got {tuple(committed.shape)}")
        if tau.shape != prefix:
            raise ValueError(f"flow time must have shape {prefix}, got {tuple(tau.shape)}")
        if len(states) != CONTROLLER_GROUP_COUNT:
            raise ValueError("flow state must contain one tensor per controller group")
        projected: list[Tensor] = []
        for group, (state, module, vocab) in enumerate(
            zip(states, self.action_projections, CONTROLLER_GROUP_VOCABS, strict=True)
        ):
            projection = cast(nn.Linear, module)
            expected = (*prefix, offsets, vocab)
            if state.shape != expected:
                raise ValueError(f"group {group} state must have shape {expected}, got {tuple(state.shape)}")
            projected.append(projection(state.to(projection.weight)))
        token = self.action_fusion_out(F.silu(self.action_fusion_in(torch.cat(projected, dim=-1))))
        offset_ids = torch.tensor(self.head_offsets, device=hidden.device)
        token = token + self.offset_embedding(offset_ids) + self.committed_embedding(committed.long())
        flat_token = token.reshape(-1, offsets, self.d_model)
        condition = self._condition(hidden, tau).reshape(-1, self.d_model)
        for block in self.blocks:
            flat_token = block(flat_token, condition)
        token = decoder_rmsnorm(flat_token).view(*prefix, offsets, self.d_model)
        if trunk_logits is None:
            trunk_logits = tuple(self.trunk_outputs[name](hidden) for name in CONTROLLER_GROUP_NAMES)
        return {
            name: center_class_logits(self.outputs[name](token) + trunk_logits[group][..., None, :])
            for group, name in enumerate(CONTROLLER_GROUP_NAMES)
        }

    @staticmethod
    def endpoint_states(targets: Tensor) -> tuple[Tensor, ...]:
        return tuple(
            F.one_hot(targets[..., group], vocab).float() for group, vocab in enumerate(CONTROLLER_GROUP_VOCABS)
        )
    ...
    def training_logits(
        ...
        endpoint = self.endpoint_states(targets)
        states = tuple(
            torch.where(
                committed[..., None],
                target,
                (1.0 - tau[..., None, None]) * z + tau[..., None, None] * target,
            )
            for target, z in zip(endpoint, noise, strict=True)
        )
        return self(hidden, states, tau, committed)
    ...
    def training_nll(
        ...
        *noise: Tensor,
        ...
        logits = self.training_logits(hidden, targets, tau, tuple(noise), committed)
    ...
    def training_statistics(
        ...
        *noise: Tensor,
        ...
        """Return endpoint CE and correctness from the same flow evaluation."""
        logits = self.training_logits(hidden, targets, tau, tuple(noise), committed)
        nll = self.nll_from_logits(logits, targets)
        correct = torch.stack(
            [logits[name].argmax(dim=-1) == targets[..., group] for group, name in enumerate(CONTROLLER_GROUP_NAMES)],
            dim=-1,
        )
        return nll, correct
    ...
    def teacher_forced_logits_by_group(
        ...
        del observed
        tau = torch.full(hidden.shape[:-1], 0.5, device=hidden.device)
        noise = tuple(torch.zeros_like(state) for state in self.endpoint_states(targets))
        committed = torch.zeros(targets.shape[:-1], dtype=torch.bool, device=targets.device)
        return self.training_logits(hidden, targets, tau, noise, committed)

    def teacher_forced_nll(self, hidden: Tensor, observed: Tensor, targets: Tensor) -> Tensor:
        return self.nll_from_logits(self.teacher_forced_logits_by_group(hidden, observed, targets), targets)
    ...
    def teacher_forced_nll_with_diagnostics(
        ...
        button_logits = logits["buttons"].detach()
        return self.nll_from_logits(logits, targets), {
            "stability/button_pre_norm_rms_min": hidden.detach().float().square().mean(dim=-1).sqrt().amin(),
            "stability/button_input_abs_p999": _sampled_quantile(hidden, 99.9, absolute=True),
            "stability/button_logit_abs_p999": _sampled_quantile(button_logits, 99.9, absolute=True),
            "stability/button_margin_mean": torch.zeros((), device=hidden.device),
        }
    ...
    @staticmethod
    def _schedule(name: str) -> tuple[tuple[float, ...], bool]:
        if name == "uniform4":
            return (0.0, 0.25, 0.5, 0.75), True
        if name == "uniform8":
            return tuple(index / 8 for index in range(8)), True
        if name == "historical4":
            return (0.0, 0.333, 0.666, 0.999), False
        raise ValueError(f"unknown flow solver {name!r}")
    ...
    def solve(
        ...
        hidden: Tensor,
        *,
        noise: tuple[Tensor, ...] | None = None,
        committed_indices: Tensor | None = None,
        committed: Tensor | None = None,
        schedule: str = "uniform4",
        generator: torch.Generator | None = None,
    ) -> FlowSolveResult:
        if hidden.ndim == 3:
            hidden = hidden[:, -1]
        if hidden.ndim != 2:
            raise ValueError("solver history must have shape [batch, width] or [batch, time, width]")
        batch = hidden.shape[0]
        offsets = len(self.head_offsets)
        if committed is None:
            committed = torch.zeros(batch, offsets, dtype=torch.bool, device=hidden.device)
        if committed.shape != (batch, offsets):
            raise ValueError("committed mask has the wrong shape")
        if committed_indices is None:
            committed_indices = torch.zeros(
                batch, offsets, CONTROLLER_GROUP_COUNT, dtype=torch.long, device=hidden.device
            )
        if committed_indices.shape != (batch, offsets, CONTROLLER_GROUP_COUNT):
            raise ValueError("committed indices have the wrong shape")
        fixed = self.endpoint_states(committed_indices)
        if noise is None:
            noise = tuple(
                torch.randn(batch, offsets, vocab, device=hidden.device, generator=generator)
                for vocab in CONTROLLER_GROUP_VOCABS
            )
        states = tuple(
            torch.where(committed[..., None], endpoint, z.float()) for endpoint, z in zip(fixed, noise, strict=True)
        )
        times, endpoint_update = self._schedule(schedule)
        trunk_logits = tuple(self.trunk_outputs[name](hidden) for name in CONTROLLER_GROUP_NAMES)
        last_logits: dict[str, Tensor] | None = None
        updates = 0
        for index, value in enumerate(times):
            tau = torch.full((batch,), value, dtype=torch.float32, device=hidden.device)
            last_logits = self(hidden, states, tau, committed, trunk_logits)
            probabilities = tuple(F.softmax(last_logits[name].float(), dim=-1) for name in CONTROLLER_GROUP_NAMES)
            should_update = endpoint_update or index < len(times) - 1
            if should_update:
                step = (1.0 / len(times)) if endpoint_update else times[index + 1] - value
                next_states: list[Tensor] = []
                for state, probability, endpoint in zip(states, probabilities, fixed, strict=True):
                    if endpoint_update and index == len(times) - 1:
                        updated = probability
                    else:
                        updated = state + step / (1.0 - value) * (probability - state)
                    next_states.append(torch.where(committed[..., None], endpoint, updated))
                states = tuple(next_states)
                updates += 1
        if last_logits is None:
            raise RuntimeError("solver schedule has no evaluations")
        if endpoint_update:
            indices = torch.stack([state.argmax(dim=-1) for state in states], dim=-1)
        else:
            indices = torch.stack([last_logits[name].argmax(dim=-1) for name in CONTROLLER_GROUP_NAMES], dim=-1)
            indices = torch.where(committed[..., None], committed_indices, indices)
        return FlowSolveResult(
            indices=indices,
            states=states,
            logits=tuple(last_logits[name] for name in CONTROLLER_GROUP_NAMES),
    ...
    def sample_indices(
        ...
        *,
        ...
        del observed, argmax, uniforms
        if offsets != self.head_offsets:
            raise ValueError(f"flow inference must solve all offsets {self.head_offsets}, got {offsets}")
        return self.solve(hidden, schedule="uniform4", generator=gen).indices
    ...
    def rollout_conditioned_logits(self, hidden: Tensor, observed: Tensor) -> tuple[list[dict[str, Tensor]], Tensor]:
        ...
        result = self.solve(
            hidden,
            noise=tuple(
                torch.zeros(batch, len(self.head_offsets), vocab, device=hidden.device)
                for vocab in CONTROLLER_GROUP_VOCABS
            ),
        )
        logits = [
            {name: result.logits[group][:, depth] for group, name in enumerate(CONTROLLER_GROUP_NAMES)}
            for depth in range(len(self.head_offsets))
        ]
        return logits, result.indices
...
class GPT(nn.Module):
    def __init__(self, cfg: TrainConfig, vocabulary: PlayerVocabulary | None = None) -> None:
        ...
        if vocabulary is not None and (
            ...
            raise ValueError("identity vocabulary does not match the frozen O57 contract")
        ...
        self.flow = CategoricalEndpointFlow(cfg, self.codec)
...
class SlotFlowRandom:
    """Gaussian plan RNG keyed by evaluator slot and match generation."""

    def __init__(self, seed: int) -> None:
        self.seed = seed & _UINT64_MASK
        self.generations: dict[int, int] = {}
        self.plan_indices: dict[tuple[int, int], int] = {}
        self.slot_ids: tuple[int, ...] = ()
        self.device = torch.device("cpu")

    def begin(self, ctx: Context) -> None:
        if ctx.slot_ids is None:
            raise ValueError("slot-keyed flow sampling needs slot_ids")
        ...
        for slot_id, reset in zip(slot_ids, resets, strict=True):
            ...
            self.plan_indices.setdefault((slot_id, self.generations[slot_id]), 0)
        self.slot_ids = slot_ids
        self.device = ctx.slot_ids.device

    def noise(self, offsets: int) -> tuple[Tensor, ...]:
        """Draw one full categorical state without depending on row order."""
        by_group: list[list[Tensor]] = [[] for _ in CONTROLLER_GROUP_NAMES]
        for slot_id in self.slot_ids:
            generation = self.generations[slot_id]
            plan_index = self.plan_indices[(slot_id, generation)]
            mixed = self.seed
            for value in (slot_id, generation, plan_index):
                mixed = _splitmix64(mixed ^ (value & _UINT64_MASK))
            generator = torch.Generator().manual_seed(mixed % ((1 << 63) - 1))
            packed = torch.randn(offsets, sum(CONTROLLER_GROUP_VOCABS), generator=generator)
            for group, values in enumerate(packed.split(CONTROLLER_GROUP_VOCABS, dim=-1)):
                by_group[group].append(values)
        return tuple(torch.stack(values).to(self.device) for values in by_group)

    def advance(self) -> None:
        for slot_id in self.slot_ids:
            key = (slot_id, self.generations[slot_id])
            self.plan_indices[key] += 1
...
def advantage_weights(
    ...
    *,
    ...
    """Return capped whole-plan AWR weights without renormalization."""
    if valid is None:
        valid = torch.ones_like(eligible)
    selected = eligible & valid
    raw = torch.exp((advantage.detach().float() / beta).clamp(max=math.log(weight_max)))
    active_weights = torch.where(selected, raw, torch.ones_like(raw))
    weights = active_weights if active else torch.ones_like(active_weights)
    selected_float = selected.float()
    count = selected_float.sum()
    denominator = count.clamp_min(1)
    selected_weights = torch.where(selected, active_weights, 0)
    weight_sum = selected_weights.sum()
    squared_sum = selected_weights.square().sum()
    ess = weight_sum.square() / (count * squared_sum).clamp_min(torch.finfo(torch.float32).tiny)
    zero = torch.zeros((), device=advantage.device)
    stats = {
        "advantage_mean": torch.where(count > 0, (advantage.float() * selected_float).sum() / denominator, zero),
        "advantage_std": zero,
        "weight_ess": torch.where(count > 0, ess, torch.ones_like(zero)),
        "weight_clip_frac": torch.where(
            count > 0,
            ((raw >= weight_max).float() * selected_float).sum() / denominator,
            zero,
        ),
        "weight_mean": torch.where(count > 0, weight_sum / denominator, torch.ones_like(zero)),
        "weight_max": torch.where(count > 0, selected_weights.max(), torch.ones_like(zero)),
        "weight_min": torch.where(
            count > 0,
            active_weights.masked_fill(~selected, float("inf")).min(),
            torch.ones_like(zero),
        ),
        "eligible_frac": count / valid.float().sum().clamp_min(1),
    }
...
def detached_value_prediction(model: GPT, hidden: Tensor) -> Tensor:
    """Predict value without allowing critic gradients into the policy trunk."""
    return model.value_head(decoder_rmsnorm(hidden).detach().float()).squeeze(-1)


@dataclass(frozen=True, slots=True)
class FlowMetricSums:
    nll_by_offset_group: Tensor
    correct_by_offset_group: Tensor
    count_by_offset: Tensor
    nll_by_time: Tensor
    correct_by_time: Tensor
    count_by_time: Tensor
    nll_by_commitment: Tensor
    correct_by_commitment: Tensor
    count_by_commitment: Tensor


def _flow_metric_sums(
    nll: Tensor,
    correct: Tensor,
    *,
    valid: Tensor,
    ...
    """Aggregate exact unweighted endpoint diagnostics without padded or fixed tokens."""
    selected = valid[..., None] & (~committed)
    selected_group = selected[..., None]
    nll_float = nll.float()
    correct_float = correct.float()

    time_bin = (tau.float() * 4).long().clamp_max(3)
    commitment_length = committed.sum(dim=-1)

    def aggregate(bins: Tensor, count: int) -> tuple[Tensor, Tensor, Tensor]:
        nll_sums = []
        correct_sums = []
        counts = []
        for index in range(count):
            mask = selected & (bins == index)[..., None]
            mask_group = mask[..., None]
            nll_sums.append(torch.where(mask_group, nll_float, 0).sum())
            correct_sums.append(torch.where(mask_group, correct_float, 0).sum())
            counts.append(mask.sum() * CONTROLLER_GROUP_COUNT)
        return torch.stack(nll_sums), torch.stack(correct_sums), torch.stack(counts)

    time_nll, time_correct, time_count = aggregate(time_bin, 4)
    commitment_nll, commitment_correct, commitment_count = aggregate(commitment_length, 5)
    return FlowMetricSums(
        nll_by_offset_group=torch.where(selected_group, nll_float, 0).sum(dim=(0, 1)),
        correct_by_offset_group=torch.where(selected_group, correct_float, 0).sum(dim=(0, 1)),
        count_by_offset=selected.sum(dim=(0, 1)),
        nll_by_time=time_nll,
        correct_by_time=time_correct,
        count_by_time=time_count,
        nll_by_commitment=commitment_nll,
        correct_by_commitment=commitment_correct,
        count_by_commitment=commitment_count,
    )


def _add_flow_metric_sums(left: FlowMetricSums, right: FlowMetricSums) -> FlowMetricSums:
    return FlowMetricSums(
        left.nll_by_offset_group + right.nll_by_offset_group,
        left.correct_by_offset_group + right.correct_by_offset_group,
        left.count_by_offset + right.count_by_offset,
        left.nll_by_time + right.nll_by_time,
        left.correct_by_time + right.correct_by_time,
        left.count_by_time + right.count_by_time,
        left.nll_by_commitment + right.nll_by_commitment,
        left.correct_by_commitment + right.correct_by_commitment,
        left.count_by_commitment + right.count_by_commitment,
    )


def _detach_flow_metric_sums(values: FlowMetricSums) -> FlowMetricSums:
    return FlowMetricSums(
        values.nll_by_offset_group.detach(),
        values.correct_by_offset_group.detach(),
        values.count_by_offset.detach(),
        values.nll_by_time.detach(),
        values.correct_by_time.detach(),
        values.count_by_time.detach(),
        values.nll_by_commitment.detach(),
        values.correct_by_commitment.detach(),
        values.count_by_commitment.detach(),
    )


def flow_objective_parts(
    nll: Tensor,
    weight: Tensor,
    committed: Tensor,
    *,
    ...
    """Average group CE over unknown offsets, then over valid prefixes."""
    unknown = ~committed
    per_offset = nll.float().mean(dim=-1)
    unknown_count = unknown.sum(dim=-1).clamp_min(1)
    plan = (per_offset * unknown).sum(dim=-1) / unknown_count
    selected_plan = torch.where(valid, plan, 0)
    total = (selected_plan * weight.float()).sum() / valid_prefixes

    selected_unknown = unknown & valid[..., None]
    near_mask = selected_unknown[..., : AWRCalibration.near_offsets]
    far_mask = selected_unknown[..., AWRCalibration.near_offsets :]
    near = (per_offset[..., : AWRCalibration.near_offsets] * near_mask).sum()
    near = near / near_mask.sum().clamp_min(1)
    far = (per_offset[..., AWRCalibration.near_offsets :] * far_mask).sum()
    far = far / far_mask.sum().clamp_min(1)
...
def microbatch_loss(
    ...
    *,
    ...
    """Compute one flow evaluation for every valid supervised prefix."""
    if not isinstance(batch, AWRBatch):
        raise TypeError(f"flow training needs an AWRBatch, got {type(batch).__name__}")
    ...
    with amp_context(cfg, DEVICE):
        ...
        if phase_timer is not None:
            phase_timer.record("flow_end")

    zero = hidden.new_zeros((), dtype=torch.float32)
    if cfg.awr_enabled:
        value = detached_value_prediction(model, hidden)
        ...
        if eligible_prefixes is not None:
            local_eligible = (batch.eligible[:, suffix_start:] & valid).sum()
            value_loss = value_loss * local_eligible / max(eligible_prefixes, 1)
        ...
        value_stats = {"value_loss": zero, "value_rmse": zero}
        stats = {
            "eligible_frac": zero,
    ...
    near, far, policy_loss = flow_objective_parts(
        dense_nll,
        weights,
        draws.committed,
    ...
    loss = policy_loss + (cfg.awr.value_loss_weight * value_loss if cfg.awr_enabled else zero)
    flow_metrics = _flow_metric_sums(
        dense_nll,
        correct,
        valid=valid,
        committed=draws.committed,
        tau=draws.tau,
    )
    flow_blocks = [cast(FlowBlock, block) for block in model.flow.blocks]
    with torch.no_grad(), amp_context(cfg, DEVICE):
        condition = model.flow._condition(hidden.detach(), draws.tau).reshape(-1, model.flow.d_model)
        gates = torch.stack(
            [torch.stack(block.modulation(condition).chunk(6, dim=-1)[2::3]).abs().mean() for block in flow_blocks]
        ).mean()
    extra = {
        ...
        "flow/committed_fraction": draws.committed.float().mean(),
        "flow/time_mean": draws.tau.mean(),
        "flow/modulation_abs_mean": gates.detach(),
    }
    if phase_timer is not None:
        phase_timer.record("objective_end")
    return loss, _detach_flow_metric_sums(flow_metrics), extra
...
def nll_mean_metrics(
    ...
    out = {
        ...
        "flow_loss_near_unweighted": float(near),
        "flow_loss_far_unweighted": float(far),
    }
...
def shuffled_history_context(context: Context, *, seed: int) -> Context:
    """Shuffle valid history before the current frame and preserve padding."""
    batch, length = next(iter(context.features.values())).shape[:2]
    indices = torch.arange(length, device=context.ctx_pad.device).expand(batch, -1).clone()
    generator = torch.Generator(device="cpu").manual_seed(seed)
    for row, pad in enumerate(context.ctx_pad.detach().cpu().tolist()):
        history_length = length - int(pad) - 1
        if history_length > 1:
            permutation = torch.randperm(history_length, generator=generator, device="cpu").to(indices.device)
            indices[row, pad : length - 1] = permutation + pad
    features = {
        name: value.gather(1, indices.view(batch, length, *(1 for _ in value.shape[2:])).expand_as(value))
        for name, value in context.features.items()
    }
    return replace(context, features=features)
...
def val_metrics(model: GPT, batches: list[TrainBatch], cfg: TrainConfig) -> dict[str, float]:
    ...
    context_sensitivity_sum = 0.0
    context_sensitivity_count = 0
    diversity_disagreements = torch.zeros_like(nll_sum)
    diversity_count = 0
    try:
        for batch_index, cpu_batch in enumerate(batches):
            ...
            with amp_context(cfg, device):
                ...
                logits = model.flow.teacher_forced_logits_by_group(hidden, history, targets)
                dense_nll = model.flow.nll_from_logits(logits, targets)
                shuffled_context = shuffled_history_context(batch.context, seed=cfg.eval_seed + batch_index)
                shuffled_hidden = model.forward_dense(
                    shuffled_context.features,
                    shuffled_context.ctx_pad,
                    None,
                )[:, -1]
            row_valid = batch.context.ctx_pad < cfg.arch.L_ctx
            if not bool(row_valid.any()):
                continue
            reference_hidden = hidden[:, -1][row_valid].float()
            shuffled_selected = shuffled_hidden[row_valid].float()
            relative_change = (reference_hidden - shuffled_selected).norm(dim=-1) / reference_hidden.norm(
                dim=-1
            ).clamp_min(1e-12)
            context_sensitivity_sum += float(relative_change.sum())
            context_sensitivity_count += len(relative_change)
            ...
            # flow and within-frame prefixes are sampled greedily.
            last_observed = history[:, -1][row_valid]
            with amp_context(cfg, device):
                rollout_logits, sampled_all = model.flow.rollout_conditioned_logits(hidden[row_valid], last_observed)
                first_plan = model.flow.solve(
                    hidden[row_valid],
                    generator=torch.Generator(device=device).manual_seed(cfg.eval_seed + 2 * batch_index + 1),
                ).indices
                second_plan = model.flow.solve(
                    hidden[row_valid],
                    generator=torch.Generator(device=device).manual_seed(cfg.eval_seed + 2 * batch_index + 2),
                ).indices
            diversity_disagreements += (first_plan != second_plan).double().sum(dim=0).cpu()
            diversity_count += len(first_plan)
    ...
    out["shuffled_context_relative_hidden_change"] = context_sensitivity_sum / max(context_sensitivity_count, 1)
    diversity_values = diversity_count * len(model.head_offsets) * CONTROLLER_GROUP_COUNT
    out["multi_noise_plan_diversity"] = float(diversity_disagreements.sum() / max(diversity_values, 1))
    for depth, offset in enumerate(model.head_offsets):
        for group, name in enumerate(CONTROLLER_GROUP_NAMES):
            out[f"plan_diversity_o{offset:02d}_{name}"] = float(
                diversity_disagreements[depth, group] / max(diversity_count, 1)
            )
...
def _validation_wandb_metrics(values: dict[str, float], cfg: TrainConfig) -> dict[str, float]:
    ...
    return {
        ...
        "far_nll": values["flow_loss_far_unweighted"],
        ...
        "shuffled_context_sensitivity": values["shuffled_context_relative_hidden_change"],
        "multi_noise_plan_diversity": values["multi_noise_plan_diversity"],
    }
...
class BF16Inference:
    ...
    def __init__(
        ...
        *,
        ...
        self.trigger_button_conflicts = 0
    ...
    def _decoder(self, bucket: int, horizon: int) -> Callable:
        ...
        if key not in self._decoders:
            ...
            def fn(hidden, committed_indices, committed, *noise):
                return self.model.flow.solve(
                    hidden,
                    noise=tuple(noise),
                    committed_indices=committed_indices,
                    committed=committed,
                    schedule=self.cfg.solver,
                ).indices
    ...
    def decode(
        ...
        *,
        ...
        stream_noise: tuple[Tensor, ...] | None = None
        if streams is not None:
            streams.begin(ctx)
            stream_noise = streams.noise(horizon)
        noise_parts: list[Tensor] = []
        for group, vocab in enumerate(CONTROLLER_GROUP_VOCABS):
            real_noise = (
                torch.randn(
                    rows,
                    horizon,
                ...
                else stream_noise[group]
            )
            noise_parts.append(F.pad(real_noise, (0, 0, 0, 0, 0, bucket - rows)))

        if committed is None:
            committed = torch.zeros(rows, horizon, dtype=torch.bool, device=ctx.ctx_pad.device)
        if committed_indices is None:
            committed_indices = torch.zeros(
                rows,
                horizon,
        ...
        padded_committed = F.pad(committed, (0, 0, 0, bucket - rows))
        padded_indices = F.pad(committed_indices, (0, 0, 0, 0, 0, bucket - rows))
        ...
        with amp_context(self.cfg, ctx.ctx_pad.device):
            ...
            indices = self._decoder(bucket, horizon)(
                hidden,
                padded_indices,
                padded_committed,
                *noise_parts,
        ...
        conflicts = ((actions[..., BUTTON_LEFT_CHANNEL] > 0.5) & (actions[..., TRIGGER_LEFT_CHANNEL] < 1.0)) | (
            (actions[..., BUTTON_RIGHT_CHANNEL] > 0.5) & (actions[..., TRIGGER_RIGHT_CHANNEL] < 1.0)
        )
        self.trigger_button_conflicts += int(conflicts.sum())
        self.decoded_frames += conflicts.numel()
        return self.model.codec.canonicalize(actions)
...
def _validate_deployment_timing(prediction_frames: int, delay_frames: int, replan_interval_frames: int) -> None:
    ...
    if delay_frames + replan_interval_frames > 6:
        raise ValueError(
            "delay_frames + replan_interval_frames must not exceed the dense six-frame prefix: "
            f"{delay_frames} + {replan_interval_frames} > 6"
        )
...
class DelayedTruncationPolicy(RecedingHorizon):
    ...
    _last_plans: dict[Slot, np.ndarray] = dataclass_field(default_factory=dict)
    neutral_actions: int = 0
    total_actions: int = 0
    overlap_frames: int = 0
    overlap_disagreements: int = 0
    replan_telemetry: DecodeTelemetry | None = None
    ...
    def __call__(self, frame_index: int, obs: Mapping[Slot, dict]):
        ...
        for slot in live:
            ...
            if state.reset_pending or slot not in self._queues:
                ...
                self._phases[slot] = 0
                self._last_plans.pop(slot, None)
        ...
        if due:
            ...
            committed = (
                np.stack([np.stack(self._queues[slot][: self.inference_delay]) for slot in due])
                if self.inference_delay
                else None
            )
            plans = self.predict_chunk(context, committed)
            ...
            for row, slot in enumerate(due):
                previous = self._last_plans.get(slot)
                if previous is not None:
                    old_overlap = previous[self.s :]
                    new_overlap = plans[row, : self.L_chunk - self.s]
                    self.overlap_frames += len(old_overlap)
                    self.overlap_disagreements += int(np.any(old_overlap != new_overlap, axis=-1).sum())
                self._last_plans[slot] = plans[row].copy()
                start = self.inference_delay
                self._queues[slot].extend(plans[row, start : start + self.s].astype(np.float32))
            if self.replan_telemetry is not None:
                self.replan_telemetry.record(
                    rows=len(due),
                    horizon=self.s,
                    seconds=time.perf_counter() - replan_started,
                )
        actions = {}
        for slot in live:
    ...
    @property
    def overlap_disagreement_fraction(self) -> float:
        return self.overlap_disagreements / max(self.overlap_frames, 1)
...
def make_policy(
    ...
    *,
    ...
    if exec_horizon is not None:
        if replan_interval_frames is not None:
            raise ValueError("pass replan_interval_frames or exec_horizon, not both")
        replan_interval_frames = exec_horizon
    ...
    def predict(ctx: Context, committed: np.ndarray | None) -> np.ndarray:
        committed_indices = None
        committed_mask = None
        if committed is not None:
            committed_actions = torch.as_tensor(committed, device=device)
            prefix = model.codec.quantize(committed_actions)
            committed_indices = torch.zeros(
                prefix.shape[0],
                horizon,
                CONTROLLER_GROUP_COUNT,
            ...
            committed_indices[:, : prefix.shape[1]] = prefix
            committed_mask = torch.zeros(prefix.shape[0], horizon, dtype=torch.bool, device=device)
            committed_mask[:, : prefix.shape[1]] = True
        result = (
            engine.decode(
...
def eval_vs_cpu(
    ...
    *,
    ...
    metrics["emulator_fps"] = sum(row.total_frames for row in rows) / max(metrics["eval_wall_seconds"], 1e-12)
    ...
    overlap_frames = sum(policy.overlap_frames for policy in policies)
    overlap_disagreements = sum(policy.overlap_disagreements for policy in policies)
    metrics["overlap_replan_frames"] = float(overlap_frames)
    metrics["overlap_replan_disagreement_fraction"] = overlap_disagreements / max(overlap_frames, 1)
    metrics["trigger_button_conflicts_before_repair"] = float(inference.trigger_button_conflicts)
    metrics["trigger_button_conflict_rate_before_repair"] = inference.trigger_button_conflicts / max(
        inference.decoded_frames, 1
    )
...
def _eval_wandb_metrics(values: dict[str, float]) -> dict[str, float]:
    ...
    names = {
        ...
        "overlap_replan_disagreement_fraction": "overlap_replan_disagreement_fraction",
        "trigger_button_conflict_rate_before_repair": "trigger_button_conflict_rate_before_repair",
...
def _final_readouts(model: GPT) -> tuple[tuple[nn.Linear, int], ...]:
    action = tuple((cast(nn.Linear, model.flow.outputs[name].down), 128) for name in CONTROLLER_GROUP_NAMES)
    trunk_skip = tuple((cast(nn.Linear, model.flow.trunk_outputs[name]), 256) for name in CONTROLLER_GROUP_NAMES)
...
def initialize_o51_parameters(model: GPT, cfg: TrainConfig) -> None:
    ...
    for module in model.flow.blocks:
        block = cast(FlowBlock, module)
...
def _is_final_readout(name: str) -> bool:
    return (
        (name.startswith("flow.outputs.") and ".down." in name)
        or name.startswith("flow.trunk_outputs.")
        or name.startswith("value_head.down.")
    )


def _output_fan_in_multiplier(name: str, cfg: TrainConfig) -> float:
    if name.startswith("flow.outputs."):
        return cfg.arch.group_head_dim / 128
    if name.startswith("flow.trunk_outputs."):
...
def optimizer_roles(model: GPT, cfg: TrainConfig) -> dict[str, OptimizerRole]:
    """Assign every parameter once by semantic optimizer role."""
    ...
    embeddings = (
        ...
        "item_state_emb.",
        "player_embedding.",
        "flow.offset_embedding.",
        "flow.committed_embedding.",
    )
    muon_exact = {
        "flow.action_fusion_in.weight": 1,
        "flow.action_fusion_out.weight": 1,
    }
    adam_matrix_prefixes = (
        "codec.semantic_projections.",
        "item_encoder.",
        "observation_encoder.",
        "player_projection.",
        "flow.action_projections.",
        "flow.history_projection.",
        "flow.time_",
        "flow.condition_",
        "flow.outputs.",
        "value_head.",
    ...
    for name, parameter in model.named_parameters():
        ...
        elif name.startswith("trunk.blocks.") and parameter.ndim == 2:
            splits = 3 if name.endswith("attn.c_attn.weight") else 2 if name.endswith("mlp.c_fc.weight") else 1
            roles[name] = OptimizerRole(cfg.optimizer, "hidden", True, logical_splits=splits)
        elif name.startswith("flow.blocks.") and parameter.ndim == 2 and ".modulation." not in name:
            splits = 3 if name.endswith("qkv.weight") else 2 if name.endswith("ffn_up.weight") else 1
            roles[name] = OptimizerRole(cfg.optimizer, "hidden", True, logical_splits=splits)
        elif name in muon_exact:
            roles[name] = OptimizerRole(cfg.optimizer, "hidden", True, logical_splits=muon_exact[name])
        elif name.startswith(embeddings):
            roles[name] = OptimizerRole("adamw", "input", False)
        elif name.startswith(adam_matrix_prefixes) or ".modulation." in name:
            kind: Literal["input", "vector"] = "input" if parameter.ndim >= 2 else "vector"
            roles[name] = OptimizerRole("adamw", kind, parameter.ndim >= 2)
        elif parameter.ndim < 2:
            roles[name] = OptimizerRole("adamw", "vector", False)
        else:
            raise RuntimeError(f"O57 has no optimizer role for {name} {tuple(parameter.shape)}")
    names = {name for name, _ in model.named_parameters()}
    if roles.keys() != names:
        raise RuntimeError(f"optimizer role mismatch: missing={names - roles.keys()}, extra={roles.keys() - names}")
    return roles
...
def make_optimizer(model: GPT, cfg: TrainConfig) -> torch.optim.Optimizer:
    """Build AdamW or Muon with auxiliary AdamW from exhaustive roles."""
    ...
    for name, role in roles.items():
        ...
        if role.optimizer == "muon":
            key = ("muon", lr, cfg.muon_weight_decay if role.decay else 0.0, role.logical_splits)
        else:
            key = ("adamw", lr, cfg.adam_weight_decay if role.decay else 0.0)
    ...
    assigned = [parameter for group in groups for parameter in cast(list[nn.Parameter], group["params"])]
    if len(assigned) != len({id(parameter) for parameter in assigned}) or len(assigned) != len(named):
        raise RuntimeError("optimizer parameters are missing or duplicated")
    if cfg.optimizer == "muon":
        return SingleDeviceMuonWithAuxAdam(groups)
    adam_groups = [
        {key: value for key, value in group.items() if key not in ("use_muon", "update_clip_threshold")}
        for group in groups
    ]
...
def _button_path_parameters(model: GPT) -> dict[str, nn.Parameter]:
    ...
    button_head = cast(NonlinearActionHead, model.flow.outputs["buttons"])
    return {
        "buttons_output_weight": cast(nn.Parameter, button_head.down.weight),
        "action_fusion_weight": cast(nn.Parameter, model.flow.action_fusion_out.weight),
        "flow_attention_weight": cast(nn.Parameter, cast(FlowBlock, model.flow.blocks[0]).qkv.weight),
    }
...
def subsystem_parameter_counts(model: GPT) -> dict[str, int]:
    partitions = _subsystem_parameter_ids(model)
...
def _subsystem_parameter_ids(model: GPT) -> dict[str, set[int]]:
    """Partition parameters by the subsystems used in the run report."""
    all_parameters = tuple(model.parameters())
    trunk_ids = {id(parameter) for parameter in model.trunk.parameters()}
    head_modules = nn.ModuleList([model.flow.outputs, model.flow.trunk_outputs])
    head_ids = {id(parameter) for parameter in head_modules.parameters()}
    flow_ids = {id(parameter) for parameter in model.flow.parameters() if id(parameter) not in head_ids}
    value_ids = {id(parameter) for parameter in model.value_head.parameters()}
    other_ids = {id(parameter) for parameter in all_parameters} - trunk_ids - flow_ids - head_ids - value_ids
    return {
        "trunk": trunk_ids,
        "flow_decoder": flow_ids,
...
def _subsystem_gradient_norms(model: GPT) -> dict[str, Tensor]:
    parameters = tuple(model.parameters())
    zero = next(iter(parameters)).new_zeros((), dtype=torch.float32)
    norms = {}
    for name, parameter_ids in _subsystem_parameter_ids(model).items():
        squares = [
            parameter.grad.detach().float().square().sum()
            for parameter in parameters
            if id(parameter) in parameter_ids and parameter.grad is not None
        ]
        norms[name] = torch.stack(squares).sum().sqrt() if squares else zero.clone()
    return norms
...
def approximate_training_flops_per_update(cfg: TrainConfig, parameter_counts: dict[str, int]) -> int:
    ...
    flow_and_heads = parameter_counts["flow_decoder"] + parameter_counts["group_heads"]
    parameter_uses = (
        full * trunk_and_inputs
        + suffix * parameter_counts["value_head"]
        + suffix * len(cfg.arch.head_offsets) * flow_and_heads
    )
...
def model_tag(cfg: TrainConfig) -> str:
    ...
    treatment = "awr-whole-plan" if cfg.awr_enabled else "no-awr"
    return (
        f"o57-{cfg.optimizer}-d{cfg.arch.d_model}-L{cfg.arch.n_layers}-h{cfg.arch.n_heads}-"
        f"Lc{cfg.arch.L_ctx}-f{cfg.arch.flow_d_model}x{cfg.arch.flow_layers}-o{offsets}-"
        f"d{cfg.delay_frames}r{cfg.replan_interval_frames}-{cfg.solver}-"
        f"alr{cfg.adam_lr:g}-mlr{cfg.muon_lr:g}-{treatment}"
    )
...
def data_selection(cfg: TrainConfig) -> PhysicalShardSelection:
    ...
    if cfg.source_names != (streams.RANKED_ANONYMIZED_POLICY_WORLD_V8[0].name,):
        raise ValueError("O57 selection requires ranked-anonymized-1-policy-world-v8")
...
def source_manifest_sha256(cfg: TrainConfig) -> dict[str, str]:
    """Return the pinned manifests for the selected rank-1 source."""
...
def source_replay_counts(cfg: TrainConfig) -> dict[str, int]:
    """Return the pinned replay counts for the selected rank-1 source."""
...
def _prepare_training_data(
    ...
    try:
        if resume_state is not None:
            ...
            if not isinstance(loader_state, dict):
                raise ValueError("resume checkpoint does not contain O57 replay-loader state")
    ...
    try:
        with ExitStack() as setup:
            executor = setup.enter_context(ThreadPoolExecutor(max_workers=1, thread_name_prefix="o57-first-batch"))
...
def _init_wandb(cfg: TrainConfig, run_name: str, resume_state: dict | None) -> None:
    ...
    wandb.init(
        ...
        tags=[
            "gpt",
            "categorical-endpoint-flow",
            cfg.optimizer,
            "scaled",
            "057",
            "projectiles",
            "ranked-anonymized-1",
            "awr" if cfg.awr_enabled else "no-awr",
            "cosine",
        ],
        config={
            **asdict(cfg),
    ...
    wandb.run.summary["architecture/treatment"] = (
        f"O57 d{cfg.arch.d_model} L{cfg.arch.n_layers} categorical endpoint flow with "
        f"ranked-anonymized-1 policy-world-v8, {cfg.optimizer}, solver={cfg.solver}, "
        f"and awr_enabled={cfg.awr_enabled}"
    )
...
def _log_training_summary(
    ...
    *,
    ...
    wandb.run.summary["training/flops_formula"] = (
        "6*B*L_ctx*(N_trunk+N_other+N_value+n_offsets*(N_flow+N_group_heads))"
    )
    wandb.run.summary["optimizer/name"] = cfg.optimizer
    wandb.run.summary["optimizer/all_parameters_use_adamw"] = cfg.optimizer == "adamw"
    wandb.run.summary["optimizer/implementation"] = (
        "torch.optim.AdamW" if cfg.optimizer == "adamw" else "SingleDeviceMuonWithAuxAdam"
    )
    ...
    wandb.run.summary["optimizer/muon_lr"] = cfg.muon_lr
...
def _training_functions(model: GPT, cfg: TrainConfig) -> tuple[Callable, Callable]:
    """Return eager or singly compiled trunk and flow training functions."""
    trunk_fn: Callable = model.forward
    flow_fn: Callable = model.flow.training_statistics
    ...
    if DEVICE == "cuda" and cfg.compile_flow:
        print(f"[compile] calling torch.compile for flow model (mode={cfg.train_compile_mode})", flush=True)
        flow_fn = torch.compile(
            flow_fn,
            dynamic=False,
            fullgraph=True,
            mode=cfg.train_compile_mode,
        )
    return trunk_fn, flow_fn


@dataclass(frozen=True, slots=True)
class TrainStepResult:
    flow_metrics: FlowMetricSums
    gradient_norm: Tensor
    metrics: dict[str, Tensor]
    adam_lr: float
    subsystem_gradients: dict[str, Tensor] = dataclass_field(default_factory=dict)
...
class _TrainingMetricAccumulator:
    ...
    def add(self, result: TrainStepResult, valid_prefixes: int) -> None:
        ...
        payload = torch.cat(
            (
                flow.nll_by_offset_group.reshape(-1).float(),
                flow.correct_by_offset_group.reshape(-1).float(),
                flow.count_by_offset.reshape(-1).float(),
                flow.nll_by_time.reshape(-1).float(),
                flow.correct_by_time.reshape(-1).float(),
                flow.count_by_time.reshape(-1).float(),
                flow.nll_by_commitment.reshape(-1).float(),
                flow.correct_by_commitment.reshape(-1).float(),
                flow.count_by_commitment.reshape(-1).float(),
                scalar_metrics,
            )
        )
    ...
    def flush(self, cfg: TrainConfig, *, update: int) -> tuple[dict[str, float], int, int]:
        ...
        offsets = len(cfg.arch.head_offsets)
        offset_group_values = offsets * CONTROLLER_GROUP_COUNT
        cursor = 0
        nll_sum = payload[cursor : cursor + offset_group_values].reshape(offsets, CONTROLLER_GROUP_COUNT)
        cursor += offset_group_values
        correct_sum = payload[cursor : cursor + offset_group_values].reshape(offsets, CONTROLLER_GROUP_COUNT)
        cursor += offset_group_values
        offset_count = payload[cursor : cursor + offsets]
        cursor += offsets
        if bool((offset_count <= 0).any()):
            raise RuntimeError(f"update {update}: an action offset has no uncommitted training examples")
        mean_nll = nll_sum / offset_count[:, None]
        mean_accuracy = correct_sum / offset_count[:, None]

        time_nll = payload[cursor : cursor + 4]
        cursor += 4
        time_correct = payload[cursor : cursor + 4]
        cursor += 4
        time_count = payload[cursor : cursor + 4]
        cursor += 4
        commitment_nll = payload[cursor : cursor + 5]
        cursor += 5
        commitment_correct = payload[cursor : cursor + 5]
        cursor += 5
        commitment_count = payload[cursor : cursor + 5]
        cursor += 5
        if bool((time_count <= 0).any()) or bool((commitment_count <= 0).any()):
            raise RuntimeError(f"update {update}: a flow diagnostic bin has no training examples")
        scalar_values = payload[cursor:] / self.updates
        nll_metrics = nll_mean_metrics(mean_nll, cfg.arch.head_offsets)
        values = {
            "train/nll": nll_metrics["loss_unweighted"],
            "optimizer/grad_norm": float(scalar_values[0]),
        }
        values.update({f"train/{name}": value for name, value in nll_metrics.items() if name.startswith("nll_o")})
        for depth, offset in enumerate(cfg.arch.head_offsets):
            for group, name in enumerate(CONTROLLER_GROUP_NAMES):
                values[f"train/acc_o{offset:02d}_{name}"] = float(mean_accuracy[depth, group])
        for index in range(4):
            values[f"flow/nll_tau_bin_{index}"] = float(scoring.nats_to_bits(time_nll[index] / time_count[index]))
            values[f"flow/acc_tau_bin_{index}"] = float(time_correct[index] / time_count[index])
        for length in range(5):
            values[f"flow/nll_committed_{length}"] = float(
                scoring.nats_to_bits(commitment_nll[length] / commitment_count[length])
            )
            values[f"flow/acc_committed_{length}"] = float(commitment_correct[length] / commitment_count[length])
...
def _slice_awr_batch(batch: AWRBatch, start: int, stop: int) -> AWRBatch:
    context = batch.context
    replay_ids = None if batch.batch.replay_ids is None else batch.batch.replay_ids[start:stop]
    return AWRBatch(
        batch=TrainBatch(
            context=Context(
        ...
        eligible=batch.eligible[start:stop],
    )
...
def train_step(
    ...
    *,
    ...
    scheduler: LambdaLR,
    phase_timer: CudaPhaseTimer | None = None,
    record_subsystem_gradients: bool = False,
) -> TrainStepResult:
    """Accumulate execution microbatches and clip once per logical batch."""
    del update
    ...
    microbatches = 0
    for start in range(0, batch.target.shape[0], cfg.execution_batch_size):
        stop = min(start + cfg.execution_batch_size, batch.target.shape[0])
        microbatch = _slice_awr_batch(batch, start, stop)
        loss, local_flow_metrics, metrics = microbatch_loss(
            model,
            microbatch,
            cfg,
            step=step,
        ...
        flow_metric_sums = (
            local_flow_metrics
            if flow_metric_sums is None
            else _add_flow_metric_sums(flow_metric_sums, local_flow_metrics)
        )
        for name, value in metrics.items():
            metric_sums[name] = metric_sums.get(name, torch.zeros_like(value)) + value
        microbatches += 1
    if flow_metric_sums is None:
        raise RuntimeError("logical batch contains no execution microbatches")
    if phase_timer is not None:
        phase_timer.record("backward_end")
    metrics = {name: value / microbatches for name, value in metric_sums.items()}
    for name in ("train/loss", "train/objective"):
        metrics[name] = metric_sums[name]
    metrics["stability/action_grad_abs_max"] = _button_gradient_abs_max(model)
    subsystem_gradients = _subsystem_gradient_norms(model) if record_subsystem_gradients else {}
    ...
    adam_lrs = [float(group["lr"]) for group in optimizer.param_groups if not bool(group.get("use_muon", False))]
    adam_lr = max(adam_lrs)
    optimizer.step()
    scheduler.step()
    if phase_timer is not None:
        phase_timer.record("optimizer_end")
    return TrainStepResult(flow_metric_sums, gradient_norm, metrics, adam_lr, subsystem_gradients)
...
def _compile_synthetic_forward_backward(
    ...
    *,
    ...
    if DEVICE == "cuda" and cfg.compile_flow:
        compile_targets.append("flow model")
    ...
    try:
        ...
        with heartbeat:
            ...
            if DEVICE == "cuda" and (cfg.compile_trunk or cfg.compile_flow):
                torch.compiler.cudagraph_mark_step_begin()
            compile_cfg = replace(cfg, batch_size=cfg.execution_batch_size)
            batch = synthetic_awr_batch(compile_cfg, torch.device(DEVICE))
            valid_prefixes = cfg.execution_batch_size * (cfg.arch.L_ctx - cfg.arch.direct_loss_start)
            loss, _flow_metrics, _metrics = microbatch_loss(
                model,
                batch,
                cfg,
                step=step,
...
def train(
    ...
    *,
    ...
    if resume_state is not None:
        validate_resume_provenance(resume_state.get("provenance"), provenance)
    ...
    start_step = 0
    actual_positions = 0
    identity_masker = IdentityMasker(cfg.seed ^ 0x0571D, cfg.identity_dropout)
    if resume_state is not None:
    ...
    trunk_fn, flow_fn = _training_functions(model, cfg)
    ...
    model.train()
    evaluation_updates = (
        frozenset(closed_loop_evaluation_updates(run_stop, cfg.eval_every))
        if cfg.automatic_evaluation
        else frozenset()
    )
    ...
    try:
        for step in range(start_step, run_stop):
            ...
            result = train_step(
                ...
                record_subsystem_gradients=metrics_due,
            )
            ...
            if metrics_due:
                ...
                log.update(
                    {
                        f"gradient/{name}_norm": float(value.detach().cpu())
                        for name, value in result.subsystem_gradients.items()
                    }
                )
...
def run_provenance(cfg: TrainConfig) -> dict[str, object]:
    """Resolve immutable code, data, optimizer, and environment identities."""
    root = Path(__file__).resolve().parents[1]
    ...
    return {
        "schema_version": 1,
        "git_sha": git_sha,
        "git_tracked_changes": bool(dirty.stdout.strip()) if dirty.returncode == 0 else None,
        "experiment_id": _EXPERIMENT_ID,
        ...
        "optimizer": {
            ...
            "matrix_weight_decay": cfg.adam_weight_decay,
            "muon_momentum": 0.95,
...
def validate_resume_provenance(stored: object, current: Mapping[str, object]) -> None:
    """Reject resume when code or persisted-input identity changed."""
    if not isinstance(stored, Mapping):
        raise ValueError("resume checkpoint has no O57 provenance record")
    ...
    changed = [name for name in identity_fields if typed_stored.get(name) != current.get(name)]
    if changed:
...
def eval_checkpoint(
    ...
    *,
    ...
    delay_frames: int | None = None,
    replan_interval_frames: int | None = None,
    solver: Literal["uniform4", "uniform8", "historical4"] | None = None,
    ...
    _validate_deployment_timing(horizon, delay, replan)
    is_variant = any(
        value is not None for value in (player_code, fixed_ego_character, delay_frames, replan_interval_frames, solver)
    )
...
def main(args: Command) -> None:
    ...
    selected_optimizer: Literal["adamw", "muon"] | None = None
    if args.arm is not None and args.proxy:
        raise SystemExit("use --arm or --proxy, not both")
    if args.resume is None and args.arm is not None:
        treatment = config_for_arm(args.arm)
        proxy_arch = treatment.arch
        target_positions = treatment.target_positions
        selected_optimizer = treatment.optimizer
    ...
    if args.resume is not None:
        ...
        resumed_arm = study_arm_for_config(cfg)
        if args.arm is not None and args.arm != resumed_arm:
            raise SystemExit(f"--arm {args.arm} does not match resumed treatment {resumed_arm!r}")
        if args.proxy and resumed_arm not in ("proxy_adamw", "proxy_muon"):
            raise SystemExit("--proxy does not match the resumed checkpoint treatment")
    ...
    cfg = replace(
        ...
        optimizer=cfg.optimizer if selected_optimizer is None else selected_optimizer,
    )
    if args.arm is not None and study_arm_for_config(cfg) != args.arm:
        raise SystemExit(
            "--arm fixes AWR, deployment timing, the primary solver, and automatic evaluation; "
            "use an unregistered run for ablations"
        )
    stats = load_stats(cfg)
    train(
        cfg,
