"""Engineered spatial observation features, forked from experiment 013.

Architecture, objective, optimizer, evaluation machinery and decode hygiene are
IDENTICAL to 013. The single studied axis is ``cfg.spatial_features``: whether the
per-frame token also carries the derived spatial block from
``hal.training.features.derive_spatial``. With it False, ``d_in``, the parameter
shapes and the forward are byte-identical to 013, so the A/B is one flag.

Motivation. 013 is measurably opponent-blind — ablating the opponent costs ~+0.15
bits against +10.8 for ablating the ego's own history — and 91% of its deaths are
at the BOTTOM blastzone, i.e. recovery and ledge failures. Schema v5 stores absolute
positions but no velocities and no stage geometry, so relative position, ledge
distance and closing speed only exist as things the trunk must reconstruct through
attention from raw ``position_x``/``position_y`` and a stage embedding. This arm
hands them over directly.

The block (23 values + 2 validity masks per frame, in ``SPATIAL_COLUMNS`` order):
  * relative — ``rel_dx``, ``rel_dy``, ``rel_dist``, and the two facing-relative
    signed magnitudes ``rel_dx_ego_facing`` / ``rel_dx_opp_facing``;
  * stage geometry per player — signed distance past the ledge, ledge-frame ``dy``,
    an offstage flag, and the four blastzone distances, all keyed by the stored
    libmelee stage id through libmelee's own ``melee.stages`` tables;
  * finite-difference velocities ``dpos_x`` / ``dpos_y`` per player — a PROXY for
    the engine velocity that schema v5 does not carry.

Seam. The block is derived inside ``preprocess`` on the RAW MDS columns, which is
the one function both ``dataloader.collate_train_batch`` and
``closed_loop.RecedingHorizon._replan`` call — so train and closed-loop eval are
the same code on the same inputs, in raw game units, with fixed documented
scalings rather than dataset statistics. Deriving model-side instead would have
had to invert the shared ego/opp position standardization (whose per-axis stds
differ) before ``rel_dist`` or any ledge comparison meant anything, and would have
put dataset constants inside the model. The pad→real boundary — where a finite
difference would otherwise read a jump from the origin — is handled inside
``derive_spatial`` via the ``spatial_dpos_mask`` sidecar; see its docstring.

Run:
    uv run experiments/016_spatial_features.py
    uv run experiments/016_spatial_features.py --cfg.no-spatial-features   # 013 baseline arm
    uv run experiments/016_spatial_features.py --eval <ckpt> --eval-temp 0.7
"""
...
@dataclass
class TrainConfig:
    # THE studied axis. True appends the derived spatial block (relative geometry, stage ledge /
    # blastzone distances, finite-difference velocities + their two validity masks) to every frame
    # token; False leaves d_in, the parameter shapes and the forward byte-identical to 013.
...
def _model_tag(cfg: TrainConfig) -> str:
    ...
    return f"gpt-d{cfg.d_model}-L{cfg.n_layers}-h{cfg.n_heads}-Lc{cfg.L_ctx}-o{offs}{spatial}"
...
def validate_config(cfg: TrainConfig, *, has_button_combo_counts: bool) -> None:
    ...
    if cfg.spatial_features and cfg.eval_incremental_kv:
        raise ValueError(
            "spatial_features needs two buffered frames for its finite-difference velocity, but the "
            "incremental decoder builds a single-frame token whose dpos would always be masked — the "
            "train and closed-loop feature distributions would differ"
        )
...
def _load_model_state(model: GPT, state_dict: dict[str, Tensor]) -> None:
    """Load 016 state, tolerating only the count buffer when inspecting an older checkpoint. A
    checkpoint trained with the other ``spatial_features`` setting has a different ``ctx_proj`` width
    and is rejected here by shape, so the two arms can never be silently interchanged."""
...
def val_metrics(model: GPT, val_cache: list[TrainBatch], cfg: TrainConfig) -> dict[str, float]:
    ...
    transition-vs-hold NLL splits, ±1-tolerant change-event F1, a copycat history-ablation probe, and — on
    the spatial arm — a spatial-ablation probe (block re-presented as fully invalid) that measures whether
    the model actually uses the engineered features. Per-element tensors are concatenated then reduced once
...
def _val_metrics_eval(model: GPT, val_cache: list[TrainBatch], cfg: TrainConfig) -> dict[str, float]:
    ...
    for batch in val_cache:
        ...
        # Spatial probe (spatial arm only): the block re-presented as fully INVALID — values zeroed, masks
        # raised — i.e. exactly the representation of an unobserved frame, not merely a zeroed one.
        h_spatial_ablated: Tensor | None = None
        ...
        for hi, o in enumerate(model.head_offsets):
            ...
            if o == 1:  # deployed head drives button / transition / ablation stats
                ...
                if h_spatial_ablated is not None:
    ...
    if spatial_ablated_cat:
        ...
        for name in _GROUP_NAMES:
            ...
            out[f"ablate_spatial_dnll_{name}"] = d  # positive ⇒ the spatial block helps
            spatial_total += d
