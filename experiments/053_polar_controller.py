"""Experiment 053: fixed polar controller codec.

This experiment compares a fixed polar stick codec with O52's Cartesian codec.
It trains AWR over ranked-anonymized-1 policy-world-v8. A learned
...
Run:
    uv run experiments/053_polar_controller.py train --proxy
    uv run experiments/053_polar_controller.py eval --checkpoint runs/<run>/final.pt
"""
...
_EXPERIMENT_ID: Final[str] = "053_polar_controller_v1"
...
CONTROLLER_GROUP_VOCABS: Final[tuple[int, ...]] = (scoring.N_BUTTON_COMBOS, 85, 13, 25)
...
MIN_NONZERO_RADIUS: Final[int] = 23
MAX_RADIUS: Final[int] = 80
MAIN_STICK_RING_ANGLES: Final[tuple[int, ...]] = (4, 16, 64)
C_STICK_RING_ANGLES: Final[tuple[int, ...]] = (4, 8)


def _polar_label_tables(angle_counts: tuple[int, ...]) -> tuple[Tensor, Tensor, Tensor, Tensor]:
    """Build fixed polar label and ring tables in HAL's [-1, 1] stick scale."""
    ring_count = len(angle_counts)
    log_min = math.log(MIN_NONZERO_RADIUS)
    log_max = math.log(MAX_RADIUS)
    raw_radii = torch.tensor(
        [
            math.exp(log_min + ring * (log_max - log_min) / (ring_count - 1))
            if ring_count > 1
            else float(MIN_NONZERO_RADIUS)
    ...
    label_angles = [0.0]
    offsets = [0, 1]
    for raw_radius, angle_count in zip(raw_radii.tolist(), angle_counts, strict=True):
        label_radii.extend([raw_radius / MAX_RADIUS] * angle_count)
        label_angles.extend([index / angle_count * 2.0 * math.pi - math.pi for index in range(angle_count)])
        offsets.append(offsets[-1] + angle_count)
    return (
        raw_radii,
        torch.tensor(label_radii, dtype=torch.float32),
        torch.tensor(label_angles, dtype=torch.float32),
...
class DiscreteControllerCodec(nn.Module):
    """Map HAL controller actions to four categorical groups with polar sticks."""

    trigger_centers: Tensor
    button_valid_for_trigger: Tensor
    main_raw_radii: Tensor
    main_label_radii: Tensor
    main_label_angles: Tensor
    main_ring_offsets: Tensor
    main_angle_counts: Tensor
    c_raw_radii: Tensor
    c_label_radii: Tensor
    c_label_angles: Tensor
    c_ring_offsets: Tensor
    c_angle_counts: Tensor
    ...
    def __init__(self, embed_dim: int) -> None:
        ...
        semantic_dims = {"buttons": 8, "main_stick": 3, "c_stick": 3, "triggers": 2}
        self.semantic_projections = nn.ModuleDict(
            {name: nn.Linear(width, embed_dim, bias=False) for name, width in semantic_dims.items()}
        )
        for prefix, angle_counts in (
            ("main", MAIN_STICK_RING_ANGLES),
            ("c", C_STICK_RING_ANGLES),
        ):
            raw_radii, label_radii, label_angles, ring_offsets = _polar_label_tables(angle_counts)
            self.register_buffer(f"{prefix}_raw_radii", raw_radii)
            self.register_buffer(f"{prefix}_label_radii", label_radii)
            self.register_buffer(f"{prefix}_label_angles", label_angles)
            self.register_buffer(f"{prefix}_ring_offsets", ring_offsets)
            self.register_buffer(f"{prefix}_angle_counts", torch.tensor((1, *angle_counts)))
    ...
    def _quantize_stick(
        ...
        raw_xy = torch.round(xy * MAX_RADIUS)
        radius = raw_xy.square().sum(dim=-1).sqrt()
        is_origin = radius <= MIN_NONZERO_RADIUS - 1
        log_fraction = (torch.log(radius + 1e-3) - math.log(MIN_NONZERO_RADIUS)) / (
            math.log(MAX_RADIUS) - math.log(MIN_NONZERO_RADIUS)
        )
        ring_count = len(angle_counts) - 1
        ring = torch.round(log_fraction * (ring_count - 1)).long()
        ring = ring.clamp(0, ring_count - 1)
        radius_bucket = torch.where(is_origin, torch.zeros_like(ring), ring + 1)
        counts = angle_counts.to(xy.device)[radius_bucket]
        normalized_angle = (torch.atan2(raw_xy[..., 1], raw_xy[..., 0]) + math.pi) / (2.0 * math.pi)
        angle_bucket = torch.round(normalized_angle * counts).long().remainder(counts)
        return ring_offsets.to(xy.device)[radius_bucket] + angle_bucket

    @staticmethod
    def _dequantize_stick(indices: Tensor, label_radii: Tensor, label_angles: Tensor) -> Tensor:
        radius = label_radii.to(indices.device)[indices]
        angle = label_angles.to(indices.device)[indices]
    ...
    def quantize(self, actions: Tensor) -> Tensor:
        ...
        buttons = scoring.buttons_to_combo(actions[..., CONTINUOUS_CHANNEL_COUNT:])
        main = self._quantize_stick(continuous[..., 0:2], self.main_ring_offsets, self.main_angle_counts)
        c_stick = self._quantize_stick(continuous[..., 2:4], self.c_ring_offsets, self.c_angle_counts)
    ...
    def dequantize(self, indices: Tensor) -> Tensor:
        ...
        main = self._dequantize_stick(indices[..., MAIN_STICK_GROUP], self.main_label_radii, self.main_label_angles)
        c_stick = self._dequantize_stick(indices[..., C_STICK_GROUP], self.c_label_radii, self.c_label_angles)
        trigger_left = scoring.center_to_value(indices[..., TRIGGERS_GROUP] // trigger_count, self.trigger_centers)
        trigger_right = scoring.center_to_value(indices[..., TRIGGERS_GROUP] % trigger_count, self.trigger_centers)
    ...
    def semantic_values(self, name: str, indices: Tensor) -> Tensor:
        ...
        if name in ("main_stick", "c_stick"):
            prefix = "main" if name == "main_stick" else "c"
            radii = cast(Tensor, getattr(self, f"{prefix}_label_radii")).to(indices.device)[indices]
            angles = cast(Tensor, getattr(self, f"{prefix}_label_angles")).to(indices.device)[indices]
            values = torch.stack((radii, torch.sin(angles), torch.cos(angles)), dim=-1)
            return torch.where((indices != 0)[..., None], values, torch.zeros_like(values))
        if name == "triggers":
            trigger_count = self.trigger_centers.shape[0]
            return torch.stack(
                (
...
class Architecture:
    ...
    def parameter_count_contract(self) -> dict[str, int]:
        if self == Architecture():
            return {
                "trunk": 201_326_592,
                "temporal_decoder": 11_296_944,
                "group_heads": 1_603_451,
...
class TrainConfig:
    ...
    materialization_threads_env: ClassVar[str] = "HAL_O53_MATERIALIZATION_THREADS"
    data_protocol: ClassVar[str] = "o53-rank1-replay-ring-v1"
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.source_names != (streams.RANKED_ANONYMIZED_POLICY_WORLD_V8[0].name,):
        raise ValueError("O53 requires only ranked-anonymized-1-policy-world-v8")
    if cfg.depth_alpha != 0.5 or cfg.hidden_std_multiplier != 0.5 or cfg.readout_init != "mup-normal":
        raise ValueError("O53 uses O50's selected depth and initialization parameterization")
    if (cfg.adam_beta1, cfg.adam_beta2, cfg.adam_eps) != (*cfg.base_adam_betas, cfg.base_adam_eps):
        raise ValueError("O53 scales the fixed base Adam betas and epsilon")
...
def depth_rule(stack: Literal["trunk", "temporal"], layers: int, alpha: float) -> DepthRule:
    ...
    if alpha != 0.5:
        raise ValueError("O53 fixes depth_alpha to 0.5")
    if layers < 1:
...
class GPT(nn.Module):
    def __init__(self, cfg: TrainConfig, vocabulary: PlayerVocabulary | None = None) -> None:
        ...
        if vocabulary is not None and (
            ...
            raise ValueError("identity vocabulary does not match the frozen O53 contract")
...
def val_metrics(model: GPT, batches: list[TrainBatch], cfg: TrainConfig) -> dict[str, float]:
    ...
    main_stick_squared = main_stick_count = 0.0
    c_stick_squared = c_stick_count = 0.0
    try:
        for cpu_batch in batches:
            ...
            main_stick_squared += float((canonical[..., 0:2] - reconstructed[..., 0:2]).square().sum())
            main_stick_count += canonical[..., 0:2].numel()
            c_stick_squared += float((canonical[..., 2:4] - reconstructed[..., 2:4]).square().sum())
            c_stick_count += canonical[..., 2:4].numel()
    ...
    out["main_stick_quantization_mse"] = main_stick_squared / max(main_stick_count, 1)
    out["c_stick_quantization_mse"] = c_stick_squared / max(c_stick_count, 1)
...
def _validation_wandb_metrics(values: dict[str, float], cfg: TrainConfig) -> dict[str, float]:
    ...
    return {
        ...
        "main_stick_quantization_mse": values["main_stick_quantization_mse"],
        "c_stick_quantization_mse": values["c_stick_quantization_mse"],
    }
    ...
    *,
    ...
    def predict(ctx: Context, committed: np.ndarray | None) -> np.ndarray:
        if committed is not None:
            raise ValueError("O53 uses truncation only and never conditions on a committed prefix")
        started = time.perf_counter()
        result = (
            engine.decode(
...
def optimizer_roles(model: GPT, cfg: TrainConfig) -> dict[str, OptimizerRole]:
    ...
    for name, parameter in model.named_parameters():
        if _is_final_readout(name):
            ...
            raise RuntimeError(f"O53 has no optimizer role for {name} {tuple(parameter.shape)}")
    return roles
...
def model_tag(cfg: TrainConfig) -> str:
    ...
    return (
        f"polar053-d{cfg.arch.d_model}-L{cfg.arch.n_layers}-h{cfg.arch.n_heads}-Lc{cfg.arch.L_ctx}-"
        f"t{cfg.arch.temporal_d_model}x{cfg.arch.temporal_layers}-o{offsets}-d2r2-"
        f"fixed-polar-sticks-nonlinear-head-trunk-skip-projectiles-v8-all-adamw-"
...
def data_selection(cfg: TrainConfig) -> PhysicalShardSelection:
    ...
    if cfg.source_names != (streams.RANKED_ANONYMIZED_POLICY_WORLD_V8[0].name,):
        raise ValueError("O53 selection requires ranked-anonymized-1-policy-world-v8")
    ...
    try:
        if resume_state is not None:
            ...
            if not isinstance(loader_state, dict):
                raise ValueError("resume checkpoint does not contain O53 replay-loader state")
...
def _init_wandb(cfg: TrainConfig, run_name: str, resume_state: dict | None) -> None:
    ...
    wandb.run.summary["nll_semantics"] = (
        ...
        "train/objective is the optimized policy-plus-value objective; NLL and class accuracy are "
        "diagnostic across O52 and O53 because their stick partitions differ"
    )
    wandb.run.summary["architecture/treatment"] = (
        f"O53 d{cfg.arch.d_model} L{cfg.arch.n_layers} AWR with ranked-anonymized-1 policy-world-v8, "
        "fixed Slippi-AI polar stick classes, O50 initialization and depth scaling, centered logits, "
        "and AdamW for every parameter"
    )
    wandb.run.summary["evaluation/selection_rule"] = (
        "gameplay net-stock bootstrap LCB, then mean net stock, then mean net damage"
    )
