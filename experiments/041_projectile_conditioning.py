"""Sparse-offset temporal controller model with projectile conditioning.

Experiment 026 is the baseline: one structured controller codec, ten selected
offsets, and a dense four- or six-frame prefix decoded online. This experiment
adds the schema-v6 projectile block (``item{0..3}_*``) to the observation.
...
Set ``--cfg.no-item-conditioning`` for the control arm. With the flag off the
architecture is identical to experiment 026.

Run:
    uv run experiments/041_projectile_conditioning.py
    uv run experiments/041_projectile_conditioning.py --eval runs/<run>/final.pt
"""
...
class TrainConfig:
    ...
    # Projectile conditioning. Off makes the architecture identical to experiment 026.
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.decode_temp != 1.0:
        raise ValueError("experiment 041 freezes sampling temperature at 1")
    ...
    if cfg.item_conditioning and cfg.observation_bundle == "v6_lean":
        raise ValueError(
            ...
            "the v6_lean bundle is dead, so projectiles condition the 'base' bundle only"
        )
...
class BF16Inference:
    ...
    def decode(
        ...
        *,
        ...
        padded = canonical_context(
            _pad_context(ctx, bucket), self.cfg.observation_bundle, items=self.cfg.item_conditioning
        )
    ...
    *,
    ...
    def predict(ctx: Context, committed: np.ndarray | None) -> np.ndarray:
        if committed is not None:
            raise ValueError("experiment 041 does not condition on a committed RTC prefix")
...
def subsystem_parameter_counts(model: GPT) -> dict[str, int]:
    observation: list[nn.Module] = [model.ctx_proj]
    if model.cfg.item_conditioning:
        observation += [model.item_type_emb, model.item_state_emb, model.item_up, model.item_down]
    groups = {
        "trunk": model.trunk,
        "observation": nn.ModuleList(observation),
        "codec": model.codec,
...
def model_tag(cfg: TrainConfig) -> str:
    ...
    return (
        f"proj041-d{cfg.d_model}-L{cfg.n_layers}-h{cfg.n_heads}-Lc{cfg.L_ctx}-"
        f"t{cfg.temporal_d_model}x{cfg.temporal_layers}-o{offsets}-s{cfg.exec_horizon}-"
        f"{cfg.observation_bundle}-{items}"
    )
...
def train(
    ...
    *,
    ...
    wandb.init(
        ...
        tags=["gpt", "temporal-mtp", "sparse-offset", "041", "projectiles"],
        config=asdict(cfg),
    )
...
def config_from_state(values: dict) -> TrainConfig:
    ...
    if missing:
        raise ValueError(f"checkpoint is not an experiment-041 architecture; missing {sorted(missing)}")
    values = dict(values)
