"""Scaled light-AWR behavior cloning with projectile conditioning.

Experiment 040 is the baseline: light AWR over the complete policy-world corpus,
with a fixed, globally calibrated weight and no learned critic. This experiment
...
architecture is identical to experiment 040.
...
_EXPERIMENT_ID = "041_projectile_conditioning_v1"
...
class TrainConfig:
    ...
    # Projectile conditioning. Off makes the architecture identical to experiment 040.
...
def _init_wandb(cfg: TrainConfig, run_name: str, resume_state: dict | None) -> None:
    ...
    wandb.init(
        ...
        tags=["gpt", "temporal-mtp", "advantage-weighted-bc", "scaled", "041", "projectiles"],
        config=asdict(cfg),
...
def config_from_state(values: dict) -> TrainConfig:
    """Restore only an explicitly identified experiment-041 checkpoint."""
