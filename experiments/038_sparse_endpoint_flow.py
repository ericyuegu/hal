"""Sparse categorical-endpoint flow action expert on the d256 causal trunk.

The observation codec, causal trunk, optimizer, data, and closed-loop protocol
match experiments 036/037. The autoregressive temporal decoder is replaced by
a bidirectional three-layer flow expert that refines all ten sparse future
actions in parallel. Training gathers 32 stratified causal prefixes after one
trunk pass; closed-loop inference uses four FP32 Euler NFEs and executes frames
one through four before replanning.

Run:
    uv run experiments/038_sparse_endpoint_flow.py
...
_EXPERIMENT_ID = "038_sparse_endpoint_flow_v1"
_INFERENCE_BUCKETS = (1, 2, 4, 8, 16, 32)
FINAL_EVAL_HORIZONS = (4,)
OFFLINE_NFES = (4, 8, 16)
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.flow_d_model // cfg.flow_heads != 64:
        raise ValueError("experiment 038 fixes the flow expert head dimension at 64")
    ...
    if cfg.exec_horizon != 4 or tuple(cfg.head_offsets[: cfg.exec_horizon]) != dense_prefix:
        raise ValueError("experiment 038 fixes exec_horizon=4 over offsets 1 through 4")
    ...
    if cfg.eval_every != 0:
        raise ValueError("experiment 038 permits closed-loop evaluation only once at the final checkpoint")
...
def _temporal_action_metrics(
    ...
    for prefix, values in (("demo", dense_target), ("flow", dense_prediction)):
        ...
        buttons = scoring.combo_to_buttons(values[..., BUTTONS_G]).bool()
        previous_buttons = scoring.combo_to_buttons(previous[..., BUTTONS_G]).bool()
...
class BF16Inference:
    ...
    def _decoder(self, bucket: int) -> Callable:
        if bucket not in self._decoders:

            def fn(hidden, noise):
                logits, _ = self.model.flow.solve(hidden[:, -1], noise, nfe=self.cfg.flow_nfe)
                return self.model.flow.argmax_indices(logits)

            self._decoders[bucket] = torch.compile(fn, dynamic=False, mode=self.compile_mode) if self.compiled else fn
        return self._decoders[bucket]
    ...
    def decode(
        ...
        *,
        ...
        if not argmax:
            raise ValueError("experiment 038 closed-loop decoding uses final endpoint argmax")
        ...
        with amp_context(self.cfg, ctx.ctx_pad.device):
            ...
            indices = self._decoder(bucket)(hidden, noise)
        if streams is not None:
        ...
        raw_actions = self.model.codec.dequantize(indices[:, :horizon])
        return self.model.codec.canonicalize(raw_actions)
    ...
    *,
    ...
    def predict(ctx: Context, committed: np.ndarray | None) -> np.ndarray:
        if committed is not None:
            raise ValueError("experiment 038 does not condition on a committed RTC prefix")
...
def _eval_protocol(
    ...
    *,
    ...
    protocol = EvalProtocol(
        ...
        action_hygiene="structured_codec_click_to_full_trigger_repair_v1",
...
def eval_vs_cpu(
    ...
    *,
    ...
    metrics["amortized_decode_ms_per_executed_frame"] = decode_metrics["decode_p50_ms"] / horizon
...
def decoder_mac_estimate(cfg: TrainConfig) -> dict[str, int]:
    """Nominal MACs for one ten-token flow-expert evaluation."""
...
def production_run_name(cfg: TrainConfig) -> str:
    return f"038-sparse-endpoint-flow-nfe{cfg.flow_nfe}-h{cfg.exec_horizon}-seed{cfg.seed}"


def model_tag(cfg: TrainConfig) -> str:
    offsets = "-".join(map(str, cfg.head_offsets))
    return (
        f"flow038-d{cfg.d_model}-L{cfg.n_layers}-h{cfg.n_heads}-Lc{cfg.L_ctx}-"
        f"f{cfg.flow_d_model}x{cfg.flow_layers}-o{offsets}-nfe{cfg.flow_nfe}-"
        f"s{cfg.exec_horizon}-{cfg.observation_bundle}"
    )
...
def train(
    ...
    *,
    ...
    wandb.init(
        ...
        tags=["gpt", "categorical-endpoint-flow", "sparse-action-plan", "038"],
        config=asdict(cfg),
    )
    if wandb.run is not None:
    ...
    try:
        ...
        final_eval = eval_vs_cpu(
            ...
            replay_dir=replay_dir / "final_h4",
            exec_horizon=cfg.exec_horizon,
            checkpoint_sha256=checkpoint_sha,
            inference=eval_inference,
        )
        values = {f"eval_h4/{name}": value for name, value in final_eval.items()}
...
def config_from_state(values: dict) -> TrainConfig:
    """Restore only an explicitly identified experiment-038 checkpoint."""
    missing = _CHECKPOINT_ARCH_FIELDS - values.keys()
    if missing:
        raise ValueError(f"checkpoint is not experiment 038; missing {sorted(missing)}")
...
def benchmark_model(model: GPT, cfg: TrainConfig, *, iterations: int, rows: int) -> dict[str, float]:
    """Measure the same batch-one full-replan latency path used by 036/037."""
