"""The schema-v6 observation bundle, forked from experiment 016.

Head, objective, optimizer, evaluation machinery and decode hygiene are the
DEPLOYED 016-base recipe — independent per-offset heads, unchanged. The one
studied axis is the FEATURE bundle the per-frame token carries. There is no flag:
016 (both arms) remains the record for the old bundle, and this file is the new
baseline.

What the token gains over 016-base (d_in 374 -> 486):

1. Engine state (schema v6). Every player block adds the five ``velocities_*``
   channels, ``state_age``, and ``misc_as``, each with its validity flag; the
   ``l_cancel`` and ``ground`` categoricals; and the per-frame ``character_live``.
   016 had to reconstruct velocity through attention over two position frames, and
   conditioned on the character-SELECT pick, which goes stale across a
   Sheik<->Zelda transform. Both are now read straight off the engine. ``misc_as``
   is action-state-multiplexed — hitstun frames remaining DURING hitstun states, an
   unrelated counter elsewhere — so the model zeroes it and raises its flag outside
   those states, exactly as it treats a missing column.

2. A LEAN spatial block: 8 of the derived block's 25 columns
   (``SPATIAL_COLUMNS_LEAN``). Offstage / ledge / bottom-blastzone target the
   measured failure budget (91% of deaths at the bottom blastzone) and two relative
   channels cover spacing. The finite-difference velocity proxy is gone — the v6
   engine velocities supersede it, which also frees the token of its second mask.

Seam. Both blocks arrive through ``hal.training.features.preprocess``, the one
function ``dataloader.collate_train_batch`` and ``closed_loop.RecedingHorizon``
share, so train and closed-loop eval are the same code on the same raw columns.
The v6 routing is declared once as ``features.V6_PLAYER_COLUMNS`` and handed to
BOTH paths; a model built before v6 passes no routing and keeps its exact width.

Run:
    uv run experiments/021_v6_features.py
    uv run experiments/021_v6_features.py --cfg.final-h2h-reference-run <016-base run>
    uv run experiments/021_v6_features.py --eval <ckpt> --eval-temp 0.7
"""
...
# Per-frame input: all four players' gamestate concatenated in the feature dim. The nana
# follower block carries the same v6 suffixes as its leader — the schema is suffix-driven.
_PLAYER_PREFIXES: tuple[str, ...] = ("ego", "ego_nana", "opp_nana", "opp")

# Per-player float channels, in token order: the 016 block first, then the v6 additions.
# Each contributes a value AND a validity flag.
_V6_FLOATS: tuple[str, ...] = tuple(V6_PLAYER_COLUMNS.floats)
_PLAYER_FLOATS: tuple[str, ...] = FLOAT_FEATURES + _V6_FLOATS
# Multiplexed by action state, so it is the one float the model gates itself (see GPT).
_MISC_AS: str = "misc_as"
assert _MISC_AS in _V6_FLOATS

# v6 categoricals that own an embedding table. ``character_live`` deliberately does not:
# it is the per-frame form of the character-SELECT pick, so it indexes ``char_emb``.
_V6_CATS: dict[str, tuple[int, int]] = {
    name: spec for name, spec in V6_PLAYER_COLUMNS.cats.items() if spec is not None
}
_CHARACTER_LIVE: str = "character_live"
assert V6_PLAYER_COLUMNS.cats[_CHARACTER_LIVE] is None

# The lean spatial block split into values and validity masks, for the ablation probe.
_LEAN_SPATIAL_VALUES: tuple[str, ...] = tuple(n for n in SPATIAL_COLUMNS_LEAN if n not in SPATIAL_MASKS)
_LEAN_SPATIAL_MASKS: tuple[str, ...] = tuple(n for n in SPATIAL_COLUMNS_LEAN if n in SPATIAL_MASKS)
...
@dataclass
class TrainConfig:
    # THE studied axis is the observation bundle, which is fixed in code (the v6 post block +
    # SPATIAL_COLUMNS_LEAN) rather than switched by a flag: 016 remains the record for the old
    # bundle, so a second arm here would only re-run it. Everything below is the deployed
    # 016-base recipe.
    # GPT backbone
    d_model: int = 256
    n_layers: int = 8
    ...
    seed: int = 0
    L_ctx: int = 256
    # optimization. batch_size / max_steps below are the DEPLOYED 016-base recipe (read back from
    # that run's checkpoint cfg), not 016's file defaults, so 021-vs-016-base is one axis.
    batch_size: int = 512
    ...
    # 016's second objection is gone: every channel here is per-frame, so a one-frame token is
    # complete (the finite-difference velocity it could not carry is now an engine column).
    ...
    final_h2h_self_label: str = "021-v6feat"
    final_h2h_n_configs: int = 64
    # checkpointing
    ckpt_every: int = 2048
    # data. The v6 re-materialization of ranked-anonymized-1: same replays and same split
    # membership as the v5 artifact 016-base trained on, so the feature bundle is the only axis.
    data_root: str = "data/processed/ranked-anonymized-1/mds-v6"
    # MDS materialization this run reads. The model consumes the v6 post block, so anything
    # older is rejected in validate_config and again by the dataloader's per-row guard.
...
def _model_tag(cfg: TrainConfig) -> str:
    ...
    return f"gpt-d{cfg.d_model}-L{cfg.n_layers}-h{cfg.n_heads}-Lc{cfg.L_ctx}-o{offs}-v6lean"
...
class GPT(nn.Module):
    ...
    def __init__(self, cfg: TrainConfig) -> None:
        ...
        # Schema-v6 categoricals with a table of their own (l_cancel, ground).
        ...
        # 13 floats (6 from 016 + 7 from v6), each with a validity flag; the 016 categoricals
        # (64+2+2+2+1); the v6 categoricals (2+4); and the per-frame live character (12).
        per_player = (
            len(_PLAYER_FLOATS) * 2
            + sum(dim for _, dim in CAT_FEATURES.values())
            + sum(dim for _, dim in _V6_CATS.values())
            + cfg.char_dim
        )  # 26 + 71 + 6 + 12 = 115
        # 4 * 115 players + 14 ego action history + 4 stage + 8 lean spatial = 486
        d_in = len(_PLAYER_PREFIXES) * per_player + A_DIM + cfg.stage_dim + len(SPATIAL_COLUMNS_LEAN)
        ...
        # Action ids whose `misc_as` slot holds the hitstun counter, resolved ONCE into a dense
        # lookup so the per-frame gate is a single gather. Registered like the center grids, so
        # it travels with the checkpoint and a later eval reads the table this run trained on.
        hitstun = torch.zeros(CAT_FEATURES["action"][0], dtype=torch.bool)
    ...
    def _per_player_features(self, features: dict[str, Tensor], prefix: str) -> Tensor:
        """One player's block: every float value, then every float's validity flag, then the
        categorical embeddings and the per-frame live character."""
        ref = features[f"{prefix}_position_x"]
        B, L = ref.shape
        device = ref.device
        # `misc_as` is multiplexed by action state: only during hitstun states is it the
        # hitstun-frames-remaining counter, elsewhere the engine reuses the slot for an
        # unrelated timer. Outside those states the value is zeroed and its flag raised —
        # the same mask-and-flag treatment a column missing from the slp gets.
        hitstun = self.hitstun_action[features[f"{prefix}_action"].clamp(0, self.hitstun_action.shape[0] - 1)]
        values: list[Tensor] = []
        masks: list[Tensor] = []
        for feat in _PLAYER_FLOATS:
            value = features[f"{prefix}_{feat}"]
            mask_key = f"{prefix}_{feat}_mask"
            mask = features[mask_key] if mask_key in features else torch.zeros(B, L, device=device)
            if feat == _MISC_AS:
                mask = torch.maximum(mask, (~hitstun).to(mask.dtype))
        ...
        # Live, transform-aware character through the SHARED character table: this replaces 016's
        # per-replay character-SELECT pick, which goes stale across a Sheik<->Zelda transform.
        live_character = features[f"{prefix}_{_CHARACTER_LIVE}"].clamp(0, self.char_emb.num_embeddings - 1)
        parts.append(self.char_emb(live_character))
        return torch.cat(parts, dim=-1)

    def _spatial_features(self, features: dict[str, Tensor]) -> Tensor:
        """The lean derived block in ``SPATIAL_COLUMNS_LEAN`` order → ``[B, L, 8]``.
        ``preprocess`` emits the full block for every batch carrying the matchup ``stage`` column,
        so an absent key means the observation source predates matchup conditioning, not a
        transient gap."""
        missing = [name for name in SPATIAL_COLUMNS_LEAN if name not in features]
        if missing:
            raise ValueError(
                f"the observation is missing the spatial columns {missing}; the source batch must "
                "carry the matchup 'stage' column for hal.training.features.preprocess to derive the block"
            )
        return torch.stack([features[name] for name in SPATIAL_COLUMNS_LEAN], dim=-1)
    ...
    def _context_tokens(self, features: dict[str, Tensor]) -> Float[Tensor, "B L_ctx d_model"]:
        ...
        # No `{ego,opp}_character` term: each player block already carries its own live character,
        # so those per-replay select-pick columns are deliberately left unread.
...
def validate_config(cfg: TrainConfig, *, has_button_combo_counts: bool) -> None:
    ...
    if cfg.mds_schema_version < 6:
        raise ValueError(
            f"021 reads the schema-v6 post block (velocities, misc_as, state_age, l_cancel, ground, "
            f"character_live), so mds_schema_version must be >= 6, got {cfg.mds_schema_version}"
        )
...
def _load_model_state(model: GPT, state_dict: dict[str, Tensor]) -> None:
    """Load 021 state, tolerating only the count buffer when inspecting an older checkpoint. A
    checkpoint from any other feature bundle (016 either arm, 019) has a different ``ctx_proj``
    width and a different set of embedding tables, so it is rejected here by shape or by key —
    the bundles can never be silently interchanged."""
...
def make_policy(
    ...
    *,
    ...
    return RecedingHorizon(
        ...
        d=0,
        device=device,
        # The same routing the train loader collates with: both paths must build one token.
...
def make_optimizer(model: GPT, cfg: TrainConfig) -> SingleDeviceMuonWithAuxAdam:
    ...
    embeds = (model.cat_embeds, model.v6_cat_embeds, model.char_emb, model.stage_emb)
...
def val_metrics(model: GPT, val_cache: list[TrainBatch], cfg: TrainConfig) -> dict[str, float]:
    ...
    transition-vs-hold NLL splits, ±1-tolerant change-event F1, a copycat history-ablation probe, and a
    spatial-ablation probe over the LEAN block (re-presented as fully invalid) that measures whether the
    model actually uses the engineered geometry. Per-element tensors are concatenated then reduced once
...
def _val_metrics_eval(model: GPT, val_cache: list[TrainBatch], cfg: TrainConfig) -> dict[str, float]:
    ...
    spatial_ablated_cat: dict[str, list[Tensor]] = {}
    ...
    for batch in val_cache:
        ...
        # Spatial probe: the LEAN block re-presented as fully INVALID — values zeroed, mask raised —
        # i.e. exactly the representation of an unobserved frame, not merely a zeroed one. Only the
        # columns the token reads are touched; the rest of the derived block is never consumed.
        spatial_ablated_features = dict(ctx.features)
        for name in _LEAN_SPATIAL_VALUES:
            spatial_ablated_features[name] = torch.zeros_like(spatial_ablated_features[name])
        for name in _LEAN_SPATIAL_MASKS:
            spatial_ablated_features[name] = torch.ones_like(spatial_ablated_features[name])
        h_spatial_ablated = model(spatial_ablated_features, ctx.ctx_pad)
        ...
        for hi, o in enumerate(model.head_offsets):
            ...
            if o == 1:  # deployed head drives button / transition / ablation stats
                ...
                spatial_logits = model.heads[model.primary_head_idx](h_spatial_ablated).float()
                spatial_kl_bits.append(_group_kl_bits(logits, spatial_logits).reshape(-1)[flat_valid])
                for name, c in group_nll(spatial_logits, tgt_idx, valid).items():
                    spatial_ablated_cat.setdefault(name, []).append(c)
                for g, name in enumerate(_GROUP_NAMES):
                    lo = _GROUP_OFFSETS[g]
    ...
    out["ablate_spatial_kl"] = torch.cat(spatial_kl_bits).mean().item()
    spatial_total = 0.0
    for name in _GROUP_NAMES:
        d = (torch.cat(spatial_ablated_cat[name]).mean() - comps[(1, name)].mean()).item() / _LN2
        out[f"ablate_spatial_dnll_{name}"] = d  # positive ⇒ the lean spatial block helps
        spatial_total += d
    out["ablate_spatial_dnll"] = spatial_total
    return out
...
def _loader_kwargs(cfg: TrainConfig, stats: dict[str, FeatureStats]) -> dict:
    ...
    materialization this run reads, so a stale (or newer) dataset raises on the first row; ``extra``
    is the v6 column routing, and it must be the same object ``make_policy`` hands the closed-loop
    policy or the two observation paths would build different tokens."""
    return dict(
