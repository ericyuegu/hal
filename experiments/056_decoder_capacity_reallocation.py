"""Experiment 056: decoder-capacity reallocation.

This experiment reallocates capacity from O52's temporal trunk into its action
decoder while holding trunk + 4 * (decoder + action heads) nearly constant. It
trains AWR over ranked-anonymized-1 policy-world-v8. A learned
...
Run:
    ...
    uv run experiments/056_decoder_capacity_reallocation.py eval --checkpoint runs/<run>/final.pt
    uv run experiments/056_decoder_capacity_reallocation.py analyze \
        --control <o52-eval-dir> --treatment <o56-eval-dir>
"""
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.source_names != (streams.RANKED_ANONYMIZED_POLICY_WORLD_V8[0].name,):
        raise ValueError("O56 requires only ranked-anonymized-1-policy-world-v8")
    if cfg.depth_alpha != 0.5 or cfg.hidden_std_multiplier != 0.5 or cfg.readout_init != "mup-normal":
        raise ValueError("O56 uses O50's selected depth and initialization parameterization")
    if (cfg.adam_beta1, cfg.adam_beta2, cfg.adam_eps) != (*cfg.base_adam_betas, cfg.base_adam_eps):
        raise ValueError("O56 scales the fixed base Adam betas and epsilon")
...
def proxy_config() -> TrainConfig:
    """Return the fixed proxy decoder-capacity treatment."""
    return TrainConfig(
        arch=Architecture(
            d_model=256,
            n_layers=11,
...
@dataclass(frozen=True, slots=True)
class BehaviorBoot:
    """Behavior and gameplay totals for one fixed evaluation boot."""

    matchup: tuple[int, int]
    wavedashes: int
    failed_wavedash_attempts: int
    behavior_minutes: float
    net_stocks: float
    net_damage: float
    gameplay_minutes: float


type RatioMetric = Literal[
...
_RATIO_METRICS: Final[tuple[RatioMetric, ...]] = (
    "failed_wavedash_attempts_per_min",
    "wavedashes_per_min",
    "wavedash_success_rate",
    "net_stock_per_min",
...
def _load_eval_evidence(eval_dir: Path) -> tuple[dict[str, object], list[MatchRow]]:
    rows_path = eval_dir / "match_rows.json"
    ...
    if not isinstance(payload, dict) or payload.get("schema_version") != _MATCH_ROW_SCHEMA_VERSION:
        raise ValueError(f"{rows_path} is not match-row schema {_MATCH_ROW_SCHEMA_VERSION}")
    raw_protocol = payload.get("protocol")
    raw_rows = payload.get("rows")
    if not isinstance(raw_protocol, dict) or not isinstance(raw_rows, list):
        raise ValueError(f"{rows_path} has invalid protocol or rows")
    ...
    if set(raw_protocol) != expected_protocol_fields:
        ...
        raise ValueError(f"{rows_path} protocol fields differ: missing={missing}, unexpected={unexpected}")
    n_matchups = raw_protocol["n_matchups"]
    if n_matchups != 96:
        raise ValueError(f"{rows_path} must describe the fixed 96-boot evaluation, got {n_matchups!r}")
    rows: list[MatchRow] = []
    for raw_row in raw_rows:
        if not isinstance(raw_row, dict):
            raise ValueError(f"{rows_path} contains a non-object match row")
        rows.append(MatchRow.from_dict(cast(Mapping[str, int | float], raw_row)))
    boots = {row.boot_index for row in rows}
    if boots != set(range(96)):
        raise ValueError(f"{rows_path} does not contain every boot 0..95: got {sorted(boots)}")
    metrics = json.loads(metrics_path.read_text())
    if not isinstance(metrics, dict) or not all(isinstance(value, (int, float)) for value in metrics.values()):
        raise ValueError(f"{metrics_path} is not a numeric metrics object")
    require_complete_eval(cast(dict[str, float], metrics), 96)
...
def _eval_evidence_sha256(eval_dir: Path) -> str:
    paths = sorted([eval_dir / "match_rows.json", eval_dir / "metrics.json", *eval_dir.glob("boot_*/*.slp")])
    digest = hashlib.sha256()
    for path in paths:
...
def _rows_by_boot(rows: Sequence[MatchRow]) -> dict[int, list[MatchRow]]:
    grouped: dict[int, list[MatchRow]] = defaultdict(list)
    for row in rows:
        grouped[row.boot_index].append(row)
    for boot_rows in grouped.values():
        boot_rows.sort(key=lambda row: row.match_ordinal)
    return grouped
...
) -> dict[int, BehaviorBoot]:
    ego_port = protocol["ego_port"]
    if ego_port not in (1, 2):
        raise ValueError(f"{eval_dir} has invalid ego_port {ego_port!r}")
    grouped = _rows_by_boot(rows)
    result: dict[int, BehaviorBoot] = {}
    for boot_index in range(96):
        boot_rows = grouped[boot_index]
        matchups = {(row.ego_character, row.opp_character) for row in boot_rows}
        if len(matchups) != 1:
            raise ValueError(f"{eval_dir} boot {boot_index} has inconsistent matchups: {sorted(matchups)}")
        matchup = next(iter(matchups))
        replay_paths = sorted((eval_dir / f"boot_{boot_index:03d}").glob("*.slp"))
        if len(replay_paths) != len(boot_rows):
            raise ValueError(
                f"{eval_dir} boot {boot_index} has {len(replay_paths)} replays for {len(boot_rows)} match rows"
            )
        wavedashes = 0
        failed = 0
        ...
        for replay_path in replay_paths:
            ...
            players = {player.port: player for player in frames.players}
            player = players[cast(int, ego_port)]
            opponent = players[2 if ego_port == 1 else 1]
            ...
            behavior_minutes += minutes
        if behavior_minutes <= 0.0:
            raise ValueError(f"{eval_dir} boot {boot_index} has no active behavior")
        gameplay_minutes = sum(row.active_frames for row in boot_rows) / (FPS * 60.0)
        if gameplay_minutes <= 0.0:
            raise ValueError(f"{eval_dir} boot {boot_index} has no recorded gameplay")
        result[boot_index] = BehaviorBoot(
            matchup=matchup,
            wavedashes=wavedashes,
            failed_wavedash_attempts=failed,
            behavior_minutes=behavior_minutes,
            net_stocks=float(sum(row.stocks_taken - row.stocks_lost for row in boot_rows)),
            net_damage=float(sum(row.damage_dealt - row.damage_taken for row in boot_rows)),
            gameplay_minutes=gameplay_minutes,
        )
    return result


def _metric_parts(boot: BehaviorBoot, metric: RatioMetric) -> tuple[float, float]:
    if metric == "failed_wavedash_attempts_per_min":
        return float(boot.failed_wavedash_attempts), boot.behavior_minutes
    if metric == "wavedashes_per_min":
        return float(boot.wavedashes), boot.behavior_minutes
    if metric == "wavedash_success_rate":
        return float(boot.wavedashes), float(boot.wavedashes + boot.failed_wavedash_attempts)
    if metric == "net_stock_per_min":
        return boot.net_stocks, boot.gameplay_minutes
    return boot.net_damage, boot.gameplay_minutes


def _ratio(numerator: np.ndarray, denominator: np.ndarray) -> float:
    total = float(denominator.sum())
    return float(numerator.sum() / total) if total > 0.0 else math.nan


def _paired_ratio_delta(
    control: Sequence[BehaviorBoot],
    treatment: Sequence[BehaviorBoot],
    metric: RatioMetric,
    *,
    sample: np.ndarray,
) -> dict[str, float | None]:
    control_parts = [_metric_parts(boot, metric) for boot in control]
    treatment_parts = [_metric_parts(boot, metric) for boot in treatment]
    control_num = np.asarray([part[0] for part in control_parts], dtype=np.float64)
    control_den = np.asarray([part[1] for part in control_parts], dtype=np.float64)
    treatment_num = np.asarray([part[0] for part in treatment_parts], dtype=np.float64)
    treatment_den = np.asarray([part[1] for part in treatment_parts], dtype=np.float64)
    control_point = _ratio(control_num, control_den)
    treatment_point = _ratio(treatment_num, treatment_den)
    point = treatment_point - control_point
    control_sample_den = control_den[sample].sum(axis=1)
    treatment_sample_den = treatment_den[sample].sum(axis=1)
    valid = (control_sample_den > 0.0) & (treatment_sample_den > 0.0)
    if valid.any():
        deltas = (
            treatment_num[sample].sum(axis=1)[valid] / treatment_sample_den[valid]
            - control_num[sample].sum(axis=1)[valid] / control_sample_den[valid]
        )
    ...
    return {
        "control": finite(control_point),
        "treatment": finite(treatment_point),
        "delta": finite(point),
        "delta_ci_lo": finite(float(lower)),
        "delta_ci_hi": finite(float(upper)),
    }
...
def analyze_evidence(
    ...
    *,
    ...
    """Validate and compare the matched O52 control and O56 treatment evidence."""
    if bootstrap_resamples < 1:
        raise ValueError("bootstrap_resamples must be positive")
    control_protocol, control_rows = _load_eval_evidence(control_dir)
    treatment_protocol, treatment_rows = _load_eval_evidence(treatment_dir)
    varying = {"checkpoint_sha256"}
    ...
    if control_invariants != treatment_invariants:
        raise ValueError("evaluation protocols differ outside checkpoint SHA-256")
    control_by_boot = _behavior_boots(control_dir, control_protocol, control_rows)
    treatment_by_boot = _behavior_boots(treatment_dir, treatment_protocol, treatment_rows)
    for boot_index in range(96):
        if control_by_boot[boot_index].matchup != treatment_by_boot[boot_index].matchup:
            raise ValueError(f"boot {boot_index} matchup differs between control and treatment")
    control = [control_by_boot[index] for index in range(96)]
    treatment = [treatment_by_boot[index] for index in range(96)]
    sample = np.random.default_rng(seed).integers(0, 96, size=(bootstrap_resamples, 96))
    metrics: dict[str, object] = {}
    for metric in _RATIO_METRICS:
        metrics[metric] = _paired_ratio_delta(control, treatment, metric, sample=sample)
    return {
        "schema_version": _ANALYSIS_SCHEMA_VERSION,
        "bootstrap_resamples": bootstrap_resamples,
        "seed": seed,
        "control_checkpoint_sha256": control_protocol["checkpoint_sha256"],
        "treatment_checkpoint_sha256": treatment_protocol["checkpoint_sha256"],
        "control_evidence_sha256": _eval_evidence_sha256(control_dir),
        "treatment_evidence_sha256": _eval_evidence_sha256(treatment_dir),
        "control_failed_wavedash_attempts": sum(boot.failed_wavedash_attempts for boot in control),
        "treatment_failed_wavedash_attempts": sum(boot.failed_wavedash_attempts for boot in treatment),
        "control_wavedashes": sum(boot.wavedashes for boot in control),
        "treatment_wavedashes": sum(boot.wavedashes for boot in treatment),
        "metrics": metrics,
    }
...
def inference_parameter_uses(parameter_counts: dict[str, int]) -> int:
    """Return the registered capacity control: trunk once and decoder four times."""
    trunk_and_inputs = parameter_counts["trunk"] + parameter_counts["other"]
    temporal_and_heads = parameter_counts["temporal_decoder"] + parameter_counts["group_heads"]
    return trunk_and_inputs + 4 * temporal_and_heads
...
def model_tag(cfg: TrainConfig) -> str:
    ...
    return (
        f"adamw056-d{cfg.arch.d_model}-L{cfg.arch.n_layers}-h{cfg.arch.n_heads}-Lc{cfg.arch.L_ctx}-"
...
def _init_wandb(cfg: TrainConfig, run_name: str, resume_state: dict | None) -> None:
    ...
    wandb.run.summary["architecture/treatment"] = (
        f"O56 d{cfg.arch.d_model} L{cfg.arch.n_layers}, temporal d{cfg.arch.temporal_d_model} "
        f"L{cfg.arch.temporal_layers} AWR with ranked-anonymized-1 policy-world-v8"
    )
...
def _log_training_summary(
    ...
    *,
    ...
    parameter_uses = inference_parameter_uses(parameter_counts)
    wandb.run.summary["parameters/inference_4x_uses"] = parameter_uses
    wandb.run.summary["parameters/control_inference_4x_uses"] = _CONTROL_INFERENCE_PARAMETER_USES
    wandb.run.summary["parameters/inference_4x_ratio"] = parameter_uses / _CONTROL_INFERENCE_PARAMETER_USES
    ...
    wandb.run.summary["training/control_approx_flops_per_update"] = _CONTROL_TRAINING_FLOPS_PER_UPDATE
    wandb.run.summary["training/approx_flops_ratio"] = flops_per_update / _CONTROL_TRAINING_FLOPS_PER_UPDATE
    wandb.run.summary["training/flops_formula"] = (
        "6*B*(L_ctx*(N_trunk+N_other)+L_suffix*N_value+L_suffix*n_offsets*(N_temporal+N_group_heads))"
    )
...
class AnalyzeArgs:
    control: Path
    treatment: Path
    out: Path = Path("o56_analysis.json")
    bootstrap_resamples: int = BOOTSTRAP_RESAMPLES
    seed: int = 0
...
def main(args: Command) -> None:
    ...
    if args.resume is None and not args.proxy:
        raise SystemExit("O56 defines only the fixed proxy treatment; pass --proxy")
