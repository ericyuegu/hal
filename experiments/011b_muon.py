"""011b: 011a + Muon optimizer (Newton-Schulz orthogonalized momentum).

Same model/objective/throughput setup as 011a (compile, b128, gated eval).
The only change is optimization: 2D hidden weight matrices (attn qkv/proj, mlp,
ctx_proj) train with Muon; embeddings, the output head, and all 1D params
(biases) stay on AdamW. Muon (Keller Jordan, modded-nanogpt) is the nanoGPT
speedrun staple — orthogonalizing the momentum update typically improves
loss-per-step. Two optimizers + two cosine scheds are bundled behind
MultiOpt/MultiSched so the train loop is unchanged.
...
class TrainConfig:
    ...
    lr: float = 3e-3  # AdamW lr for the embedding/head/bias group (Muon trains the matrices)
    muon_lr: float = 0.02  # Muon lr for 2D hidden weight matrices
...
def _model_tag(cfg: TrainConfig) -> str:
    return f"gpt-muon-d{cfg.d_model}-L{cfg.n_layers}-h{cfg.n_heads}-Lc{cfg.L_ctx}"
...
@torch.compile
def _zeropower_ns5(G: Tensor, steps: int = 5, eps: float = 1e-7) -> Tensor:
    """Quintic Newton-Schulz orthogonalization of a 2D matrix, run in bf16 (Keller
    Jordan's coefficients, maximizing slope at 0). Returns ~UV^T of G's SVD."""
    assert G.ndim == 2
...
class Muon(torch.optim.Optimizer):
    """SGD-momentum whose update matrix is orthogonalized via Newton-Schulz. Only
    for 2D hidden weights — not embeddings, the output head, or 1D params."""

    def __init__(
        self, params, lr: float = 0.02, momentum: float = 0.95, nesterov: bool = True, ns_steps: int = 5
    ) -> None:
    ...
    def step(self) -> None:
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if g.size(0) == 3 * g.size(1):  # fused QKV: orthogonalize Q, K, V separately
                    g = torch.cat([_zeropower_ns5(s, ns_steps) for s in g.split(g.size(1))])
                    scale = g.size(1) ** 0.5
                else:
                    g = _zeropower_ns5(g, ns_steps)
                    scale = max(g.size(0), g.size(1)) ** 0.5  # update.square().mean() ≈ 1
                p.add_(g, alpha=-lr * scale)


class MultiOpt:
    """Bundle several optimizers behind one .zero_grad/.step/.state_dict so the
    train loop can treat Muon+AdamW as a single optimizer."""

    def __init__(self, *opts: torch.optim.Optimizer) -> None:
        self.opts = opts
    ...
    def load_state_dict(self, sd: dict) -> None:
        for o, s in zip(self.opts, sd["opts"], strict=True):
            o.load_state_dict(s)


class MultiSched:
    """Step several LR schedulers together (one per bundled optimizer)."""

    def __init__(self, *scheds: LambdaLR) -> None:
        self.scheds = scheds
    ...
    def load_state_dict(self, sd: dict) -> None:
        for s, st in zip(self.scheds, sd["scheds"], strict=True):
            s.load_state_dict(st)


def split_muon_adamw(model: nn.Module) -> tuple[list, list]:
    """2D hidden matrices → Muon; embeddings, head, and all 1D params → AdamW."""
    muon, adamw = [], []
    for name, p in model.named_parameters():
        if not p.requires_grad:
            continue
        if p.ndim == 2 and "emb" not in name and "lm_head" not in name:
            muon.append(p)
        else:
            adamw.append(p)
    return muon, adamw
...
def train(
    ...
    *,
    ...
    opt_muon = Muon(muon_params, lr=cfg.muon_lr, momentum=0.95, nesterov=True)
    opt_adamw = AdamW(adamw_params, lr=cfg.lr, betas=(0.9, 0.95), weight_decay=cfg.weight_decay)
    opt = MultiOpt(opt_muon, opt_adamw)
    sched = MultiSched(LambdaLR(opt_muon, lr_schedule(cfg)), LambdaLR(opt_adamw, lr_schedule(cfg)))
    print(
        f"[opt] Muon on {sum(p.numel() for p in muon_params) / 1e6:.2f}M params, "
        f"AdamW on {sum(p.numel() for p in adamw_params) / 1e6:.2f}M params",
        flush=True,
    )
