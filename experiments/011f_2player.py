"""011f: 011e (relfeat + AR-groups) but 2-player input — drop the nana slots.

The recent 009 commit added nana (4-player: ego, ego_nana, opp_nana, opp). But
the eval is Fox-vs-Fox, where both nana slots are ALWAYS absent (masked zeros) —
2 of 4 per-player feature blocks are dead weight that may dilute the model and
explain why the 4-player baseline (13.4%) trails the pre-nana 20% SOTA. This
variant uses only (ego, opp), the only players present in the evaluated matchup
(legitimate for the Fox-ditto metric; it would only matter for Ice-Climbers
matchups, which aren't evaluated). Otherwise identical to 011e.

011d: 010 (AR-over-groups head) + torch.compile + AdamW + gated eval.

Systems treatment identical to 011a (compile the training fwd, beartype decorators
stripped, eval/val cadence gated, push_to_r2 off). The MODEL is 010's
autoregressive-groups head: within each frame, group g conditions on the realized
classes of groups 0..g-1 — fixing 009's conditional independence that 007 argued
shreds closed-loop cross-channel coherence. AdamW (Muon lost in 011b).
...
_PLAYER_PREFIXES: tuple[str, ...] = ("ego", "opp")  # 2-player: drop nana (absent in Fox dittos)

# Nonlinear ego↔opp relative channels appended to every context token (from 011c; helped winrate).
...
class TrainConfig:
    ...
    compile: bool = True  # torch.compile the training forward (teacher-forced head)
    # eval cadence
    val_every: int = 1024
...
def _model_tag(cfg: TrainConfig) -> str:
    return f"gpt-ar2p-d{cfg.d_model}-L{cfg.n_layers}-h{cfg.n_heads}-dg{cfg.d_group}-Lc{cfg.L_ctx}"
...
# %%
def eval_temp_sweep(ckpt_path: str, temps: tuple[float, ...], *, replicas: int, max_frames: int) -> None:
    """Lean test-time decode-temperature sweep on FD vs lvl-9 CPU (no retraining). Prints pooled
    vs-cpu metrics + stock winrate per temp; a cheap screen for the best deploy temperature."""
    model, cfg, stats, state = _load_ckpt(ckpt_path)
    replay_dir = Path(ckpt_path).resolve().parent / "temp_sweep_replays"
    replay_dir.mkdir(parents=True, exist_ok=True)
    session_cfg = default_session_cfg(replay_dir)
    print(f"[temps] {ckpt_path} step={state['step']} replicas={replicas} frames={max_frames}", flush=True)
    for t in temps:
        results = sweep_vs_cpu(
            lambda t=t: make_policy(model, stats, cfg, decode_temp=t),
            session_cfg=session_cfg,
            stages=(melee.Stage.FINAL_DESTINATION,),
            replicas=replicas,
            max_parallel=cfg.eval_max_parallel,
        ...
        m = vs_cpu_metrics(results)
        taken, lost = m.get("stocks_taken_per_min", 0.0), m.get("stocks_lost_per_min", 0.0)
        wr = taken / (taken + lost) if (taken + lost) else 0.0
        print(
            f"[temps] temp={t:.2f}  winrate={wr:.3f}  taken={taken:.3f} lost={lost:.3f} "
            f"dealt={m.get('damage_dealt_per_min', 0):.1f} dmg_taken={m.get('damage_taken_per_min', 0):.1f} "
            f"crashed={m.get('crashed', 0):.2f}",
            flush=True,
        )
...
class Args:
    ...
    eval_temp_sweep: str | None = None  # ckpt path; lean FD temp sweep instead of train
    sweep_temps: str = "1.0,1.2,1.4"  # comma-sep decode temps for --eval-temp-sweep
    sweep_replicas: int = 8
    sweep_frames: int = 5400
...
def main(args: Args) -> None:
    if args.eval is not None:
        ...
        return
    if args.eval_temp_sweep is not None:
        temps = tuple(float(x) for x in args.sweep_temps.split(","))
        eval_temp_sweep(args.eval_temp_sweep, temps, replicas=args.sweep_replicas, max_frames=args.sweep_frames)
        return
