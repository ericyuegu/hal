...
_EXPERIMENT_ID = "036_advantage_weighted_bc_v1"
...
class TrainConfig:
    ...
    # True keeps the treatment pure reweighting: the value MSE trains only the
    # value head. False lets it also shape the trunk (a second treatment axis).
    awr_value_detach_trunk: bool = True
...
def model_tag(cfg: TrainConfig) -> str:
    ...
    treatment = f"awr-{cfg.advantage_scope}-b{cfg.awr_beta:g}-g{cfg.awr_gamma:g}-wu{cfg.awr_warmup_steps}"
    if cfg.temporal_state_film:
        treatment = f"{treatment}-film"
    return (
        f"mtp036-d{cfg.d_model}-L{cfg.n_layers}-h{cfg.n_heads}-Lc{cfg.L_ctx}-"
...
def train(
    ...
    *,
    ...
    wandb.init(
        ...
        tags=["gpt", "temporal-mtp", "advantage-weighted-bc", "036"],
        config=asdict(cfg),
    )
    if wandb.run is not None:
        ...
        wandb.run.summary["nll_semantics"] = (
            "advantage weights change the backprop objective only; "
            "train/loss and every validation metric stay unweighted"
        )
