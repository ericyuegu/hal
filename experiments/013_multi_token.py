"""Validity-hardened multi-token action policy, forked from experiment 012.
...
013 deliberately leaves the default 012 learning objective and model unchanged. It fixes configuration validation,
evaluation protocol reproducibility, dataset-scoped button-support metadata, and chunk execution across asynchronous
instant-restart boundaries. See ``docs/experiments/013_multi_token.md`` for the frozen follow-up ablations.

Run:
    uv run experiments/013_multi_token.py
    uv run experiments/013_multi_token.py --cfg.head-offsets 1 5 9 13
    uv run experiments/013_multi_token.py --eval <ckpt> --eval-temp 0.7
    # Chunk execution requires a checkpoint trained with the contiguous heads it executes:
    uv run experiments/013_multi_token.py --eval <ckpt-with-o1.2> --eval-exec-horizon 2
"""
...
def _load_model_state(model: GPT, state_dict: dict[str, Tensor]) -> None:
    """Load 013 state, tolerating only the new count buffer when inspecting an older checkpoint."""
