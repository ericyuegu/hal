"""O60: one plain-BC counterpart to O52 run 0ksityvv.

Keep O52 initialization, batches, optimizer, and near/far normalization.
Policy weights are always one. The allocated critic is never evaluated or
trained, so it contributes no gradient to global clipping. This tests the
full AWR package, not advantage weighting alone.
"""
...
_EXPERIMENT_ID: Final[str] = "060_plain_bc_v1"
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.awr.value_loss_weight != 0:
...
def microbatch_loss(
    ...
    *,
    ...
    """Apply the unchanged near/far objective with unit weights at every update."""
    ...
    extra = {
        ...
        "awr/active": torch.zeros_like(loss),
        "awr/weight_mean": torch.ones_like(loss),
        "awr/weight_max": torch.ones_like(loss),
        **button_diagnostics,
    }
    if phase_timer is not None:
...
def validate_control_invariants(cfg: TrainConfig) -> None:
    control_path = Path(__file__).with_name("o60") / "control-config.json"
    expected = json.loads(control_path.read_text())
    expected["experiment_id"] = _EXPERIMENT_ID
    expected["awr_calibration"]["value_loss_weight"] = 0.0
    actual = json.loads(json.dumps(_checkpoint_config(cfg)))
    if actual != expected:
        changed = sorted(name for name in expected.keys() | actual.keys() if expected.get(name) != actual.get(name))
        raise ValueError(f"O60 control invariants changed: {changed}")


def validate_control_protocol(protocol: EvalProtocol) -> None:
    path = Path(__file__).with_name("o60") / "control-match-rows.json"
    expected = json.loads(path.read_text())["protocol"]
    actual = asdict(protocol)
    expected["checkpoint_sha256"] = actual["checkpoint_sha256"]
    if actual != expected:
        changed = sorted(name for name in expected.keys() | actual.keys() if expected.get(name) != actual.get(name))
        raise ValueError(f"corrected control evaluation protocol changed: {changed}")
...
def eval_vs_cpu(
    ...
    *,
    ...
    validate_control_protocol(protocol)
...
def model_tag(cfg: TrainConfig) -> str:
    ...
    treatment = f"plain-bc-unit-weights-no-critic-wu{cfg.warmup_steps}"
    return (
        f"adamw060-d{cfg.arch.d_model}-L{cfg.arch.n_layers}-h{cfg.arch.n_heads}-Lc{cfg.arch.L_ctx}-"
...
def restore_rng_state(state: dict) -> None:
    if set(state) != {"python", "numpy", "torch", "cuda"}:
        raise ValueError("incomplete process RNG state")
    random.setstate(state["python"])
    np.random.set_state(state["numpy"])
    torch.set_rng_state(state["torch"].cpu())
    if len(state["cuda"]) != torch.cuda.device_count():
        raise ValueError("CUDA device count changed across resume")
    if state["cuda"]:
...
def write_provenance(
    ...
    source = streams.BY_NAME[cfg.source_names[0]]
    expected = {
        "stats.json": "6870bda6c0970826f6467407647cf9bb5901ef246b6947d87518db67252cdde3",
        "train/index.json": "b97eab90e761bcf2bf03b48981f0ab6acc1ac3057157c58ae0c5a72c76c43bd8",
        "val/index.json": "95738bf46604a7e7b67320095efbcc2dbe1a8ae748aa6a269e19f0cee52da389",
    }
    actual = {name: checkpoint_sha256(source.local_root / name) for name in expected}
    if actual != expected:
        raise ValueError(f"control data hashes differ: {actual}")
    cohort = hashlib.sha256()
    for batch in validation:
        cohort.update(repr(batch.replay_ids).encode())
        for name, value in sorted(batch.context.features.items()):
            cohort.update(name.encode())
            cohort.update(value.numpy().tobytes())
        cohort.update(batch.context.ctx_pad.numpy().tobytes())
        cohort.update(batch.target.numpy().tobytes())
    payload = {
        "git_sha": os.environ.get("HAL_GIT_SHA"),
        "experiment_source_sha256": checkpoint_sha256(Path(__file__)),
        "control_run": "ericyuegu/hal/0ksityvv",
        "control_git_sha": None,
        "control_checkpoint_sha256": "16c702fe3964a59c2f26d88207ef90137d93c07bf67a5d4213f4fde5d25b8631",
        "config": _checkpoint_config(cfg),
        "data_sha256": actual,
        "validation_sha256": cohort.hexdigest(),
        "resolved_stats": {name: asdict(value) for name, value in stats.items()},
        ...
        "seeds": {"training": cfg.seed, "identity_mask": cfg.seed ^ 0x0521D, "evaluation": cfg.eval_seed},
    }
    if not payload["git_sha"]:
        raise ValueError("HAL_GIT_SHA is required for treatment provenance")
...
def _init_wandb(cfg: TrainConfig, run_name: str, resume_state: dict | None) -> None:
    ...
    wandb.run.summary["nll_semantics"] = (
        ...
        "train/objective is the policy-only objective; the critic has no gradient"
    )
    wandb.run.summary["architecture/treatment"] = (
        f"O60 d{cfg.arch.d_model} L{cfg.arch.n_layers} plain BC with ranked-anonymized-1 policy-world-v8, "
...
def train(
    ...
    *,
    ...
    if cfg.automatic_evaluation:
        raise ValueError("O60 requires one separate final evaluation")
    ...
    if not smoke:
        validate_control_invariants(cfg)
    ...
    write_provenance(run_dir, cfg, stats, prepared_data.validation, uploader)
...
def eval_checkpoint(
    ...
    *,
    ...
    if expected_checkpoint_sha256 is None:
        raise ValueError("final evaluation requires the verified checkpoint SHA-256")
    ...
    cfg = config_from_state(dict(source.checkpoint_config))
    validate_config(cfg)
    ...
    if update != 16_384:
        raise ValueError("O60 evaluates only update 16,384")
    if n_matchups not in (None, 96):
        raise ValueError("O60 requires the full 96-boot suite")
