"""O58: condition the O55 add-history decoder on the next 60 frames' return.

Standalone copy of the frozen control. Future-return labels never enter the
observation encoder, trunk, value targets, or replay sampling geometry.
"""
...
_EXPERIMENT_ID: Final[str] = "058_return_conditioning_v1"
_STARTUP_LOG_INTERVAL_S: Final[float] = 60.0
_MATCH_ROW_SCHEMA_VERSION: Final[int] = 8
...
CONTROL_CHECKPOINT_SHA256: Final[str] = "e8ef7228d0fb0197e31e328fee6b0667b768878b7f0cb3d803da0abdb35c5136"
CONTROL_SOURCE_SHA256: Final[str] = "71f59435ae8c85921dd05c948310d91e2cd6b283c58f37285c5e04542291b514"
CONTROL_RUN: Final[str] = (
    "260914-014038_055_history_cross_attention_attention055-d256-L16-h4-Lc256-t128x4-"
    "o1-2-3-4-5-6-9-12-16-20-d2r2-add-history-final-prefix-all-adamw-alr0.0017-g0.99855_"
    "ranked-anon-1_o55-b-add-history"
)
...
def conditioning_protocol() -> dict[str, object]:
    return {
        ...
        "evaluation": "fixed request at every replan; zero, positive median, positive p90",
...
class ReturnBatch:
    ...
    def record_stream(self, stream: torch.cuda.Stream) -> None:
        ...
        for tensor in (self.returns, self.eligible, self.future_return, self.available):
...
class ReturnCalibration:
    ...
    def observe(self, batch: ReturnBatch) -> None:
        ...
        valid = batch.available[:count].detach().cpu()
        values = torch.where(valid, batch.future_return[:count].detach().cpu(), 0.0)
    ...
    def state_dict(self) -> dict[str, object]:
        data = {"values": self.values, "valid": self.valid, "replay_ids": self.replay_ids}
    ...
    def load_state_dict(self, state: dict[str, object]) -> None:
        ...
        self.values = list(cast(list[float], state["values"]))
...
def validate_conditioning_state(state: dict) -> None:
    if state.get("checkpoint_version") != 1:
        raise ValueError("incompatible O58 checkpoint version")
    if state.get("conditioning_protocol") != conditioning_protocol():
        raise ValueError("incompatible conditioning protocol")
    if not isinstance(state.get("rng"), dict) or not isinstance(state.get("return_calibration"), dict):
        raise ValueError("checkpoint lacks RNG or return calibration state")


@functools.cache
def execution_identity() -> dict[str, object]:
    root = Path(__file__).resolve().parents[1]
    source_files = [Path(__file__).resolve(), root / "uv.lock", *sorted((root / "hal").rglob("*.py"))]
    git_sha = os.environ.get("HAL_GIT_SHA")
    if git_sha is None:
        git_sha = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=root, text=True).strip()
    return {
        "git_sha": git_sha,
        "source_sha256": {str(path.relative_to(root)): checkpoint_sha256(path) for path in source_files},
        "python": sys.version,
        "packages": {
            "torch": torch.__version__,
            "numpy": np.__version__,
            "scipy": scipy.__version__,
            "mosaicml-streaming": streaming.__version__,
            "melee": melee.version.__version__,
        },
...
def validate_control_config(cfg: TrainConfig) -> None:
    """Reject scientific drift from the verified historical control."""
    ...
    changed = [
        ...
        if item.name not in operational and getattr(cfg, item.name) != getattr(expected, item.name)
    ]
    if changed:
        raise ValueError(f"configuration differs from registered control: {changed}")
...
class TrainConfig:
    ...
    decoder_variant: DecoderVariant = "add-history"
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.source_names != (streams.RANKED_ANONYMIZED_POLICY_WORLD_V8[0].name,):
        raise ValueError("O58 requires only ranked-anonymized-1-policy-world-v8")
    if cfg.depth_alpha != 0.5 or cfg.hidden_std_multiplier != 0.5 or cfg.readout_init != "mup-normal":
        raise ValueError("O58 uses O50's selected depth and initialization parameterization")
    if (cfg.adam_beta1, cfg.adam_beta2, cfg.adam_eps) != (*cfg.base_adam_betas, cfg.base_adam_eps):
        raise ValueError("O58 scales the fixed base Adam betas and epsilon")
...
def synthetic_awr_batch(cfg: TrainConfig, device: torch.device) -> ReturnBatch:
    ...
    return ReturnBatch(
        ...
        returns=returns,
        eligible=eligible,
        future_return=torch.zeros(cfg.batch_size, device=device),
        available=torch.ones(cfg.batch_size, dtype=torch.bool, device=device),
    )
...
def depth_rule(stack: Literal["trunk", "temporal"], layers: int, alpha: float) -> DepthRule:
    ...
    if alpha != 0.5:
        raise ValueError("O58 fixes depth_alpha to 0.5")
    if layers < 1:
...
class CausalTemporalDecoder(nn.Module):
    """The four registered O58 action-decoder variants."""
    ...
    def configure_return_projection(self) -> None:
        with torch.random.fork_rng(devices=[]):
            torch.random.default_generator.manual_seed(self.cfg.seed ^ 0x058A77E)
            self.return_projection = nn.Linear(1, self.d_model, bias=False)
            _initialize_hidden_module(self.return_projection, self.cfg)

    def return_bias(self, desired_return: Tensor, available: Tensor) -> Tensor:
        if desired_return.ndim != 1 or available.shape != desired_return.shape or available.dtype != torch.bool:
            raise ValueError("return and availability must be [B] float and bool tensors")
        value = torch.where(available, desired_return, 0.0) / RETURN_SCALE
        return self.return_projection(value[:, None])
    ...
    def _decode_step(
        ...
        state = decoder_rmsnorm(state_bias + self._step_features(previous, offsets) + return_bias)
    ...
    def teacher_forced_states(
        ...
        x = decoder_rmsnorm(x[:, 0] + self.return_bias(desired_return, available)[:, None])
    ...
    def _teacher_forced_outputs(
        ...
        states = self.teacher_forced_states(hidden, ctx_pad, observed, targets, desired_return, available)
    ...
    def teacher_forced_logits_by_group(
        ...
        logits, _ = self._teacher_forced_outputs(hidden, ctx_pad, observed, targets, desired_return, available)
        return logits
    ...
    def teacher_forced_nll(
        ...
        logits = self.teacher_forced_logits_by_group(hidden, ctx_pad, observed, targets, desired_return, available)
    ...
    def teacher_forced_nll_with_diagnostics(
        ...
        logits, button_values = self._teacher_forced_outputs(
            hidden, ctx_pad, observed, targets, desired_return, available
        )
    ...
    def teacher_forced_logits(
        ...
        values = self.teacher_forced_logits_by_group(hidden, ctx_pad, observed, targets, desired_return, available)
        return [
    ...
    def forced_stepwise_logits(
        ...
        histories = self._project_histories(hidden, ctx_pad)
        condition = self.return_bias(desired_return, available)
        previous = observed
        caches: list[tuple[Tensor, Tensor] | None] = [None] * len(self.blocks)
        out: list[dict[str, Tensor]] = []
        for depth, offset in enumerate(self.head_offsets):
            state, caches = self._decode_step(previous, offset, state_bias, caches, histories, condition)
...
class GPT(nn.Module):
    def __init__(self, cfg: TrainConfig, vocabulary: PlayerVocabulary | None = None) -> None:
        ...
        if vocabulary is not None and (
            ...
            raise ValueError("identity vocabulary does not match the frozen O58 contract")
        ...
        self.temporal.configure_return_projection()
        self.calibration = ReturnCalibration()
...
def collate_awr_batch(windows: list[dict], batch: TrainBatch, *, L_ctx: int) -> ReturnBatch:
    ...
    return ReturnBatch(
        ...
        future_return=torch.tensor([window["ego_return60"][L_ctx - 1] for window in windows]),
        available=torch.tensor([window["ego_return60_valid"][L_ctx - 1] for window in windows], dtype=torch.bool),
    )
    ...
    *,
    ...
    with amp_context(cfg, DEVICE):
        ...
        temporal_output = temporal_fn(
            hidden, batch.context.ctx_pad, history, targets, batch.future_return, batch.available
        )
    ...
    extra = {
        ...
        "return60/available_fraction": batch.available.float().mean(),
        ...
        for cpu_batch in batches:
            ...
            with amp_context(cfg, device):
                rollout_logits, sampled_all = model.temporal.rollout_conditioned_logits(
                    hidden[row_valid],
                    batch.context.ctx_pad[row_valid],
                    last_observed,
                    batch.future_return[row_valid],
                    batch.available[row_valid],
                )
...
@torch.no_grad()
def conditioning_diagnostic(model: GPT, batches: list[ReturnBatch], cfg: TrainConfig) -> dict[str, float]:
    """Shuffle only valid labels across the fixed validation cohort, with a local RNG."""
    values = torch.cat([batch.future_return.cpu() for batch in batches])
    available = torch.cat([batch.available.cpu() for batch in batches])
    valid_indices = available.nonzero().flatten()
    if len(valid_indices) < 2:
        raise ValueError("conditioning diagnostic requires at least two valid returns")
    generator = torch.Generator().manual_seed(58)
    shuffled = values.clone()
    shuffled[valid_indices] = values[valid_indices[torch.randperm(len(valid_indices), generator=generator)]]
    actual_sum = shuffled_sum = 0.0
    count = cursor = 0
    device = next(model.parameters()).device
    for cpu_batch in batches:
        batch = cpu_batch.to(device)
        size = len(batch.available)
        history, targets, valid = prepared_targets(model, batch)
        selected = batch.available & valid[:, 0]
        with amp_context(cfg, device):
            hidden = model.forward_dense(batch.context.features, batch.context.ctx_pad, None)
            actual = model.temporal.teacher_forced_nll(
                hidden,
                batch.context.ctx_pad,
                history,
                targets,
            ...
            permuted = model.temporal.teacher_forced_nll(
                hidden,
                batch.context.ctx_pad,
                history,
                targets,
                shuffled[cursor : cursor + size].to(device),
                batch.available,
            )
        actual_sum += float(actual[selected].double().sum())
        shuffled_sum += float(permuted[selected].double().sum())
        count += actual[selected].numel()
        cursor += size
    valid_returns = values[available].numpy()
    return {
        "return60_actual_nll_bits": actual_sum / count / math.log(2),
        "return60_shuffled_nll_bits": shuffled_sum / count / math.log(2),
        "return60_available_fraction": float(available.float().mean()),
        "return60_mean": float(valid_returns.mean()),
        "return60_p10": float(np.quantile(valid_returns, 0.1)),
        "return60_median": float(np.quantile(valid_returns, 0.5)),
        "return60_p90": float(np.quantile(valid_returns, 0.9)),
    }
...
def _validation_wandb_metrics(values: dict[str, float], cfg: TrainConfig) -> dict[str, float]:
    ...
    return {
        ...
        **{name: value for name, value in values.items() if name.startswith("return60_")},
    }
...
class BF16Inference:
    ...
    def __init__(
        ...
        *,
        ...
        desired_return: float | None = 0.0,
    ) -> None:
        self.model = model
        self.cfg = cfg
        if desired_return is not None and not math.isfinite(desired_return):
            raise ValueError("desired return must be finite")
    ...
    def _decoder(self, bucket: int, horizon: int) -> Callable:
        ...
        if key not in self._decoders:
            ...
            def fn(hidden, ctx_pad, observed, uniforms, desired_return, available):
                return self.model.temporal.sample_indices(
                    hidden, ctx_pad, observed, offsets, desired_return, available, argmax=False, uniforms=uniforms
                )
    ...
    def decode(
        ...
        *,
        ...
        desired_return = torch.full((bucket,), self.desired_return or 0.0, device=ctx.ctx_pad.device)
        available = torch.full((bucket,), self.desired_return is not None, dtype=torch.bool, device=ctx.ctx_pad.device)
        ...
        with amp_context(self.cfg, ctx.ctx_pad.device):
            ...
            if argmax:
                ...
                indices = self._decoder(bucket, horizon)(
                    hidden, padded.ctx_pad, observed[:, -1], uniforms, desired_return, available
                )
    ...
    *,
    ...
    def predict(ctx: Context, committed: np.ndarray | None) -> np.ndarray:
        if committed is not None:
            raise ValueError("O58 uses truncation only and never conditions on a committed prefix")
        started = time.perf_counter()
        result = (
            engine.decode(
...
class EvalProtocol:
    ...
    conditioning_protocol: dict[str, object]
...
def _eval_protocol(
    ...
    *,
    ...
    return EvalProtocol(
        ...
        calibration_sha256=str(model.calibration.state_dict()["sha256"]) if desired_return is not None else None,
...
def _load_match_row_evidence(path: Path) -> tuple[dict[str, object], list[MatchRow]]:
    ...
    if not isinstance(payload, dict) or payload.get("schema_version") != _MATCH_ROW_SCHEMA_VERSION:
        raise ValueError(f"{path} is not O58 match-row schema {_MATCH_ROW_SCHEMA_VERSION}")
...
def analyze_paired_evidence(
    ...
    *,
    ...
    varying = {"checkpoint_sha256", "decoder_variant", "desired_return", "calibration_sha256"}
...
def eval_vs_cpu(
    ...
    *,
    ...
    if inference.desired_return != desired_return:
        raise ValueError("inference target differs from evaluation protocol")
...
def optimizer_roles(model: GPT, cfg: TrainConfig) -> dict[str, OptimizerRole]:
    ...
    for name, parameter in model.named_parameters():
        if _is_final_readout(name):
            ...
            or (name.startswith("temporal.outputs.") and name.endswith("up.weight"))
            or name == "value_head.up.weight"
        ):
            ...
            raise RuntimeError(f"O58 has no optimizer role for {name} {tuple(parameter.shape)}")
    return roles
...
def subsystem_parameter_counts(model: GPT) -> dict[str, int]:
    ...
    added += model.cfg.arch.temporal_d_model
...
def data_selection(cfg: TrainConfig) -> PhysicalShardSelection:
    ...
    if cfg.source_names != (streams.RANKED_ANONYMIZED_POLICY_WORLD_V8[0].name,):
        raise ValueError("O58 selection requires ranked-anonymized-1-policy-world-v8")
    ...
    *,
    ...
    save_checkpoint(
        ...
        extra_state={
            ...
            "rng": rng_state(),
            "conditioning_protocol": conditioning_protocol(),
            "return_calibration": model.calibration.state_dict(),
...
def _collate_o58_batch(
    ...
    *,
    ...
    return ReturnBatch(
        ...
        torch.from_numpy(np.ascontiguousarray(columns["ego_return60"][:, context_length - 1])),
        torch.from_numpy(np.ascontiguousarray(columns["ego_return60_valid"][:, context_length - 1])).bool(),
    )
    ...
    try:
        if resume_state is not None:
            ...
            if not isinstance(loader_state, dict):
                raise ValueError("resume checkpoint does not contain O58 replay-loader state")
...
def _init_wandb(cfg: TrainConfig, run_name: str, resume_state: dict | None) -> None:
    ...
    wandb.run.summary["architecture/treatment"] = (
        f"O58 {cfg.decoder_variant} decoder with final-prefix AWR on ranked-anonymized-1 "
...
def train(
    ...
    *,
    ...
    if not smoke and (
        cfg.arch != proxy_config().arch or cfg.target_windows != 2**23 or cfg.batch_size != 512 or cfg.seed != 0
    ):
        raise ValueError("O58 permits only the registered single-seed proxy")
    if cfg.decoder_variant != "add-history" or cfg.awr != AWRCalibration() or cfg.automatic_evaluation:
        raise ValueError("O58 requires add-history, the control AWR objective, and final target-suite evaluation")
    ...
    if resume_state is not None:
        ...
        validate_conditioning_state(resume_state)
        if resume_state.get("execution_identity") != execution_identity():
            raise ValueError("resume source or environment differs from the checkpoint")
        model.calibration.load_state_dict(resume_state["return_calibration"])
    ...
    try:
        for step in range(start_step, run_stop):
            ...
            model.calibration.observe(batch)
            if phase_timer is not None:
...
def load_checkpoint(path: str, *, device: str = DEVICE) -> tuple[GPT, TrainConfig, dict[str, FeatureStats], dict]:
    ...
    validate_conditioning_state(state)
    ...
    model.calibration.load_state_dict(state["return_calibration"])
    model.eval()
...
def eval_checkpoint(
    ...
    *,
    ...
    if target_index not in (0, 1, 2):
        raise ValueError("target_index must be zero, median, or p90 (0, 1, 2)")
    desired_return = model.calibration.targets()[target_index]
    cfg = replace(cfg, eval_seed=eval_seed)
    validate_config(cfg)
...
def load_control_checkpoint(path: Path, *, device: str = DEVICE) -> tuple[GPT, TrainConfig, dict[str, FeatureStats]]:
    if checkpoint_sha256(path) != CONTROL_CHECKPOINT_SHA256:
        raise ValueError("control checkpoint hash differs from verified kahz2d77 final.pt")
    state = torch.load(path, map_location=device, weights_only=False)
    if (
        state["step"] != 16_383
        or state["wandb_id"] != "kahz2d77"
        or state["cfg"]["experiment_id"] != "055_history_cross_attention_v1"
    ):
        raise ValueError("control checkpoint identity mismatch")
    cfg = config_from_state({**state["cfg"], "experiment_id": _EXPERIMENT_ID})
    validate_control_config(cfg)
    vocabulary = PlayerVocabulary(decode_player_codes(state["model"]["player_code_bytes"].cpu().numpy().tobytes()))
    model = GPT(cfg, vocabulary).to(device)
    missing, unexpected = model.load_state_dict(state["model"], strict=False)
    if missing != ["temporal.return_projection.weight"] or unexpected:
        raise ValueError(f"control model parameters differ: missing={missing}, unexpected={unexpected}")
    model.eval()
    return model, cfg, load_stats(cfg)


def validated_eval_rows(path: Path, expected: EvalProtocol, *, historical: bool = False) -> list[MatchRow]:
    payload = json.loads(path.read_text())
    if payload.get("schema_version") != (7 if historical else _MATCH_ROW_SCHEMA_VERSION):
        raise ValueError("evaluation schema mismatch")
    protocol = dict(payload["protocol"])
    wanted = asdict(expected)
    if historical:
        for name in ("desired_return", "conditioning_protocol", "calibration_sha256"):
            wanted.pop(name)
    # CPU visibility does not affect the fixed 32-slot evaluation schedule.
    for name in ("allowed_cpus", "hardware_wave_bucket"):
        protocol.pop(name)
        wanted.pop(name)
    if protocol != wanted:
        changed = sorted(name for name in protocol.keys() | wanted.keys() if protocol.get(name) != wanted.get(name))
        raise ValueError(f"evaluation protocol mismatch: {changed}")
    rows = [MatchRow.from_dict(row) for row in payload["rows"]]
    totals = _boot_stock_totals(rows)
    if set(totals) != set(range(_REGISTERED_MATCHUPS)):
        raise ValueError("evaluation must contain all 96 boots")
    schedule = matchups_for_vs_cpu(_REGISTERED_MATCHUPS)
    for boot, (ego, opponent) in enumerate(schedule):
        if totals[boot][0] != (int(ego.value), int(opponent.value), expected.seed_stage):
            raise ValueError(f"evaluation boot {boot} matchup differs from the registered schedule")
    return rows


def select_return_target(comparisons: Sequence[dict[str, float]]) -> int:
    if len(comparisons) != 3:
        raise ValueError("selection requires all three frozen targets")
    if any(not math.isfinite(row["treatment_net_stock_per_min"]) for row in comparisons):
        raise ValueError("target selection contains a nonfinite rate")
    # Stable ties prefer zero, then the median, then p90.
    return max(range(3), key=lambda index: comparisons[index]["treatment_net_stock_per_min"])


def evaluate_suite(
    checkpoint: Path, control_checkpoint: Path, *, saved_control: Path | None = None, upload_run: str | None = None
) -> dict[str, object]:
    model, cfg, stats, state = load_checkpoint(str(checkpoint))
    validate_control_config(cfg)
    if state["step"] != 16_383 or state["actual_supervised_prefixes"] != 2**23:
        raise ValueError("target selection requires the final 16,384-update treatment")
    control, control_cfg, control_stats = load_control_checkpoint(control_checkpoint)
    targets = model.calibration.targets()
    treatment_sha = checkpoint_sha256(checkpoint)
    root = checkpoint.resolve().parent / "return-evaluation-v1"
    root.mkdir(parents=True, exist_ok=True)
    registration = {
        "schema_version": 1,
        "targets": list(targets),
        "calibration_sha256": model.calibration.state_dict()["sha256"],
        "treatment_checkpoint_sha256": treatment_sha,
        "control_checkpoint_sha256": CONTROL_CHECKPOINT_SHA256,
        "control_source_sha256": CONTROL_SOURCE_SHA256,
        "conditioning_protocol": conditioning_protocol(),
    }
    registration_path = root / "frozen-targets.json"
    ...
    def evaluate(
        arm: GPT, arm_cfg: TrainConfig, arm_stats: dict[str, FeatureStats], name: str, target: float | None, seed: int
    ) -> list[MatchRow]:
        arm_cfg = replace(arm_cfg, eval_seed=seed)
        sha = CONTROL_CHECKPOINT_SHA256 if target is None else treatment_sha
        expected = _eval_protocol(
            arm_cfg, arm, n_matchups=96, checkpoint_sha256=sha, desired_return=target, inference_compile_mode="default"
        )
        directory = root / name
        rows_path = directory / "match_rows.json"
        if target is None and seed == 0 and saved_control is not None and not rows_path.exists():
            try:
                rows = validated_eval_rows(saved_control, expected, historical=True)
            except ValueError as error:
                print(f"[eval] saved control rejected; reevaluating verified checkpoint: {error}", flush=True)
            else:
                _write_eval_evidence(directory, rows, {}, expected)
        if not rows_path.exists():
            metrics = eval_vs_cpu(
                ...
                desired_return=target,
            )
            require_complete_eval(metrics, 96)
        rows = validated_eval_rows(rows_path, expected)
        if upload_run is not None:
            _upload_eval_evidence(upload_run, root)
        return rows

    control_rows = evaluate(control, control_cfg, control_stats, "selection-control-seed0", None, 0)
    selection = []
    for index, target in enumerate(targets):
        rows = evaluate(model, cfg, stats, f"selection-target{index}-seed0", target, 0)
        selection.append(paired_net_stock_delta(control_rows, rows, bootstrap_resamples=2_000, seed=0))
    selected = select_return_target(selection)
    selection_payload = {
        **registration,
    ...
    if selection_path.exists() and json.loads(selection_path.read_text()) != selection_payload:
        raise ValueError("target selection changed on continuation")
    selection_path.write_text(json.dumps(selection_payload, indent=2) + "\n")
    if upload_run is not None:
        _upload_eval_evidence(upload_run, root)
    control_rows = evaluate(control, control_cfg, control_stats, "confirmation-control-seed1", None, 1)
    treatment_rows = evaluate(model, cfg, stats, f"confirmation-target{selected}-seed1", targets[selected], 1)
    confirmation = {
        **registration,
        "selected_index": selected,
        "seed": 1,
        "comparison": paired_net_stock_delta(control_rows, treatment_rows, bootstrap_resamples=2_000, seed=1),
    }
    (root / "confirmation.json").write_text(json.dumps(confirmation, indent=2) + "\n")
    if upload_run is not None:
        _upload_eval_evidence(upload_run, root)
    print(json.dumps({"selection": selection_payload, "confirmation": confirmation}, indent=2), flush=True)
    return confirmation
...
class SuiteArgs:
    checkpoint: str
    control_checkpoint: str = "final.pt"
    control_run: str | None = CONTROL_RUN
    saved_control: str | None = None
    run: str | None = None
...
type Command = (
    Annotated[SuiteArgs, tyro.conf.subcommand(name="suite")]
...
def main(args: Command) -> None:
    if isinstance(args, SuiteArgs):
        checkpoint = _resolve_eval_checkpoint(args.checkpoint, args.run)
        control_checkpoint = _resolve_eval_checkpoint(args.control_checkpoint, args.control_run)
        evaluate_suite(
            checkpoint,
            control_checkpoint,
            saved_control=None if args.saved_control is None else Path(args.saved_control),
            upload_run=args.run,
        )
        return
