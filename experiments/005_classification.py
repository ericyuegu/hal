...
Euler integration. Buttons are 8 independent Bernoullis (``multi_label``, primary) or a
9-way "none-or-one" categorical (``single_label``, diagnostic — only honest if the
``--audit`` multipress rate is small). Continuous channels are per-axis bins
(``naive_bins``) or joint-2D stick clusters + trigger bins (``stick_clusters``); the
discretizers live in ``hal.training.scoring`` so targets, decode, and metrics share one
quantizer.
...
class ClassifierPolicy(nn.Module):
    ...
    def __init__(self, cfg: TrainConfig):
        ...
        if cfg.button_head not in ("multi_label", "single_label"):
            raise ValueError(f"button_head must be multi_label|single_label, got {cfg.button_head!r}")
        ...
        self.trig_binspecs = self.cont_binspecs[4:6]
        if cfg.continuous_head == "naive_bins":
            ...
            self.trig_head = nn.Linear(dh, 2 * cfg.n_bins)
        self.button_head = nn.Linear(dh, _N_BUTTONS if cfg.button_head == "multi_label" else _N_BUTTONS + 1)
    ...
    def logits(self, cond: Float[Tensor, "N d_model"]) -> dict[str, Tensor]:
        ...
        if self.continuous_head_kind == "naive_bins":
            ...
            out["c"] = self.c_head(h)
            out["trig"] = self.trig_head(h).reshape(N, H, 2, self.n_bins)
...
def _component_nll(model: ClassifierPolicy, logits: dict[str, Tensor], tgt: Tensor) -> dict[str, Tensor]:
    ...
    if model.continuous_head_kind == "naive_bins":
        ...
        tce = _ce_nats(logits["trig"], scoring.bins_to_idx(cont_tgt[..., 4:6], model.trig_binspecs))  # [N,H,2]
        comps["triggers"] = tce[..., 0] + tce[..., 1]
...
def decode(
    ...
    *,
    ...
    if model.continuous_head_kind == "naive_bins":
        ...
        trig = scoring.idx_to_centers(pick(lg["trig"]), model.trig_binspecs)
...
def _button_marginal_probs(model: ClassifierPolicy, button_logits: Tensor) -> Tensor:
    """P(button_k pressed) ∈ [0,1], ``[..., 8]`` — sigmoid for multi-label, the per-button
    softmax marginal (classes 1..8) for single-label. Lets both heads be scored as a proper
    Bernoulli model of the buttons."""
    if model.button_head_kind == "multi_label":
...
def run_audit(cfg: TrainConfig, stats: dict[str, FeatureStats], *, n_batches: int = 16) -> None:
    ...
    print(f"  {'discretizer':<22} {'main_mae':>9} {'cstick_mae':>11} {'trig_mae':>9} {'>0.1 frac':>10}", flush=True)
    for nb in (15, 21, 31):
        specs = scoring.cont_binspecs(nb)
        recon = scoring.idx_to_centers(scoring.bins_to_idx(cont, specs), specs)
        ...
        print(
            f"  {'naive_bins(' + str(nb) + ')':<22} {main_mae:>9.4f} {c_mae:>11.4f} {trig_mae:>9.4f} {far:>10.3f}",
            flush=True,
        )

    main_c, c_c = scoring.STICK_CLUSTER_CENTERS_MAIN, scoring.STICK_CLUSTER_CENTERS_C
    main_recon = scoring.cluster_to_xy(scoring.nearest_cluster(cont[:, 0:2], main_c), main_c)
    c_recon = scoring.cluster_to_xy(scoring.nearest_cluster(cont[:, 2:4], c_c), c_c)
    far = (main_recon - cont[:, 0:2]).abs().max(-1).values.gt(0.1).float().mean().item()
    print(
        f"  {'stick_clusters(' + str(main_c.shape[0]) + ')':<22} {_mae(main_recon, cont[:, 0:2]):>9.4f} "
        f"{_mae(c_recon, cont[:, 2:4]):>11.4f} {'-':>9} {far:>10.3f}",
        flush=True,
    )
