"""Four-frame action-chunk IQL on the sparse-offset temporal controller.
...
selected offsets, and decodes only the dense four-frame prefix online.

The policy is experiment 026 unchanged.  Training adds a categorical
teacher-forced chunk-Q head and a scalar high-expectile value head; closed-loop
inference still executes only the existing policy's next four frames.

Run:
    uv run experiments/027_temporal_mtp.py
    uv run experiments/027_temporal_mtp.py --eval runs/<run>/final.pt
"""
...
WANDB_NOTES = """Closed-loop evaluation is the main result.
Falling Q/V losses do not prove policy improvement.
TD error measures Bellman consistency.
Large or drifting Q/V scales indicate critic instability.
Advantage near zero means behavior approaches BC.
Very large advantage tails cause actor-weight concentration.
High weight clip fraction means w_max is controlling training.
corr(Q,G) and corr(V,G) are sanity checks only.
Do not optimize V-vs-G MAE; high-expectile V is intentionally not the mean behavior return.
Expectile balance near 1 means the V objective is behaving as intended."""
...
class TrainConfig:
    ...
    # Four-frame chunk-IQL.  These values are in the shaped stock-return units
    # defined below, not action-log-loss bits.
    iql_tau: float = 0.9
    iql_gamma: float = 0.99827
    iql_beta: float = 0.8
    iql_weight_max: float = 5.0
    iql_q_min: float = -4.0
    iql_q_max: float = 4.0
    iql_q_bins: int = 201
    iql_q_target_encoding: str = "two_hot"  # or hl_gauss
    iql_hl_gauss_sigma_ratio: float = 0.75
    iql_q_loss_coefficient: float = 1.0
    iql_value_loss_coefficient: float = 1.0
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.exec_horizon != EXECUTED_CHUNK:
        raise ValueError("experiment 027 always replans after exactly four executed frames")
    ...
    if not math.isfinite(cfg.iql_tau) or not 0.0 < cfg.iql_tau < 1.0:
        raise ValueError("iql_tau must be finite and in (0, 1)")
    if not math.isfinite(cfg.iql_gamma) or not 0.0 <= cfg.iql_gamma <= 1.0:
        raise ValueError("iql_gamma must be finite and in [0, 1]")
    for name in ("iql_beta", "iql_weight_max", "iql_hl_gauss_sigma_ratio"):
        value = getattr(cfg, name)
        if not math.isfinite(value) or value <= 0.0:
            raise ValueError(f"{name} must be finite and positive")
    if cfg.iql_q_bins < 2:
        raise ValueError("iql_q_bins must be at least 2")
    if not math.isfinite(cfg.iql_q_min) or not math.isfinite(cfg.iql_q_max) or cfg.iql_q_min >= cfg.iql_q_max:
        raise ValueError("iql_q_min and iql_q_max must be finite and increasing")
    if cfg.iql_q_target_encoding not in ("two_hot", "hl_gauss"):
        raise ValueError("iql_q_target_encoding must be 'two_hot' or 'hl_gauss'")
    for name in ("iql_q_loss_coefficient", "iql_value_loss_coefficient"):
        value = getattr(cfg, name)
        if not math.isfinite(value) or value < 0.0:
            raise ValueError(f"{name} must be finite and non-negative")
    for name in ("iql_damage_shaping", "iql_win_reward"):
        if not math.isfinite(getattr(cfg, name)):
...
class GPT(nn.Module):
    def __init__(self, cfg: TrainConfig) -> None:
        ...
        # Construct critics only after every 026 policy parameter.  With the
        # same seed, all policy draws therefore remain byte-for-byte comparable.
        chunk_width = EXECUTED_CHUNK * N_GROUPS * cfg.action_embed_dim
        self.q_head = NonlinearActionHead(cfg.d_model + chunk_width, cfg.group_head_dim, cfg.iql_q_bins)
        self.value_head = nn.Linear(cfg.d_model, 1)
        self.register_buffer("q_bin_values", torch.linspace(cfg.iql_q_min, cfg.iql_q_max, cfg.iql_q_bins))
    ...
    def value(self, hidden: Tensor) -> Tensor:
        return self.value_head(decoder_rmsnorm(hidden)).float().squeeze(-1)

    def chunk_q_logits(self, hidden: Tensor, chunk: Tensor) -> Tensor:
        if chunk.shape != (*hidden.shape[:-1], EXECUTED_CHUNK, N_GROUPS):
            raise ValueError(
                f"Q needs a teacher-forced [B, L, {EXECUTED_CHUNK}, {N_GROUPS}] chunk, "
                f"got {tuple(chunk.shape)} for hidden {tuple(hidden.shape)}"
            )
        action = self.codec.embed_frame(chunk).flatten(start_dim=-2)
...
def label_iql_replay(sample: dict, *, gamma: float, damage_shaping: float, win_reward: float) -> dict:
    out = dict(sample)
...
class IQLBatch:
    batch: TrainBatch
    extended_context: Context  # [B, L_ctx + 4], includes the observed next state
    rewards: Tensor  # [B, L_ctx, 4], r_{t+1:t+4}
    returns: Tensor  # [B, L_ctx], actual Monte-Carlo G_t for validation only

    def to(self, device: str | torch.device) -> IQLBatch:
        return IQLBatch(
            batch=self.batch.to(device),
            extended_context=self.extended_context.to(device),
    ...
    def pin_memory(self) -> IQLBatch:
        return IQLBatch(
            ...
            extended_context=self.extended_context.pin_memory(),
    ...
    def take(self, count: int) -> IQLBatch:
        return IQLBatch(
            batch=TrainBatch(
                context=Context(
                    features={name: value[:count] for name, value in self.batch.context.features.items()},
                    ctx_pad=self.batch.context.ctx_pad[:count],
                ),
...
def encode_q_target(target: Tensor, model: GPT, cfg: TrainConfig) -> Tensor:
    bins = model.q_bin_values.float()
    value = target.detach().float()
    if cfg.iql_q_target_encoding == "two_hot":
        clipped = value.clamp(float(bins[0]), float(bins[-1]))
        position = (clipped - bins[0]) / (bins[1] - bins[0])
        lower = position.floor().long().clamp(0, bins.numel() - 1)
        upper = position.ceil().long().clamp(0, bins.numel() - 1)
        upper_weight = position - lower.float()
        lower_weight = 1.0 - upper_weight
        encoded = torch.zeros(*value.shape, bins.numel(), device=value.device, dtype=torch.float32)
        encoded.scatter_add_(-1, lower[..., None], lower_weight[..., None])
        encoded.scatter_add_(-1, upper[..., None], upper_weight[..., None])
        return encoded
    if cfg.iql_q_target_encoding == "hl_gauss":
        spacing = bins[1] - bins[0]
        sigma = spacing * cfg.iql_hl_gauss_sigma_ratio
        midpoints = 0.5 * (bins[:-1] + bins[1:])
        edges = torch.cat((bins.new_tensor([float("-inf")]), midpoints, bins.new_tensor([float("inf")])))
        standardized = (edges - value[..., None]) / sigma
        cdf = 0.5 * (1.0 + torch.erf(standardized / math.sqrt(2.0)))
        encoded = (cdf[..., 1:] - cdf[..., :-1]).clamp_min(0.0)
        return encoded / encoded.sum(dim=-1, keepdim=True).clamp_min(torch.finfo(encoded.dtype).tiny)
    raise ValueError(f"unknown Q target encoding {cfg.iql_q_target_encoding!r}")


def actor_weights(advantage: Tensor, *, beta: float, weight_max: float) -> tuple[Tensor, Tensor]:
    if advantage.requires_grad:
        raise ValueError("actor weights require a detached advantage")
    scaled = advantage.float() / beta
...
class IQLParts:
    ...
    advantage: Tensor
    weight: Tensor
    weight_clipped: Tensor
    td_error: Tensor
    q_target_oob: Tensor


def chunk_td_target(rewards: Tensor, next_value: Tensor, *, gamma: float) -> Tensor:
    if rewards.shape[:-1] != next_value.shape or rewards.shape[-1] != EXECUTED_CHUNK:
        raise ValueError("chunk TD target needs rewards [..., 4] and one aligned next value per chunk")
    discounts = rewards.new_tensor([gamma**index for index in range(EXECUTED_CHUNK)])
    return (rewards.float() * discounts).sum(dim=-1) + gamma**EXECUTED_CHUNK * next_value.detach()
...
def iql_parts(
    ...
    q_logits_grid = model.chunk_q_logits(hidden, targets[..., :EXECUTED_CHUNK, :])
    q_logits = q_logits_grid[valid]
    q = model.decode_q(q_logits)
    value_grid = model.value(hidden)
    next_value_grid = model.value(next_hidden)
    value = value_grid[valid]
    target_grid = chunk_td_target(rewards, next_value_grid, gamma=cfg.iql_gamma)
    target = target_grid[valid].detach()
    encoded = encode_q_target(target, model, cfg)
    q_loss = -(encoded * F.log_softmax(q_logits.float(), dim=-1)).sum(dim=-1).mean()

    delta = q.detach() - value
    expectile_weight = torch.where(delta < 0, 1.0 - cfg.iql_tau, cfg.iql_tau)
    value_loss = (expectile_weight * delta.square()).mean()
    advantage = (q.detach() - value.detach()).detach()
    weight, clipped = actor_weights(advantage, beta=cfg.iql_beta, weight_max=cfg.iql_weight_max)

    joint_nll = dense_nll[valid].sum(dim=-1)
    weighted_primary = (weight[:, None] * joint_nll[:, :EXECUTED_CHUNK]).mean()
    unweighted_primary = joint_nll[:, :EXECUTED_CHUNK].mean()
    auxiliary = joint_nll[:, EXECUTED_CHUNK:].mean()
    return IQLParts(
        policy_loss=weighted_primary + cfg.aux_loss_weight * auxiliary,
        unweighted_policy_loss=unweighted_primary + cfg.aux_loss_weight * auxiliary,
        q_loss=q_loss,
        ...
        weight_clipped=clipped.detach(),
        td_error=(target - q.detach()),
        q_target_oob=((target < cfg.iql_q_min) | (target > cfg.iql_q_max)).detach(),
    )
...
def _distribution_metrics(name: str, values: Tensor) -> dict[str, float]:
    ...
    quantiles = torch.quantile(x, x.new_tensor([0.05, 0.5, 0.95]))
    return {
        f"{name}_mean": x.mean().item(),
        f"{name}_std": x.std(unbiased=False).item(),
        f"{name}_p05": quantiles[0].item(),
        f"{name}_p50": quantiles[1].item(),
        f"{name}_p95": quantiles[2].item(),
    }


def _correlation(left: Tensor, right: Tensor) -> float:
    x = left.detach().double()
    y = right.detach().double()
    x = x - x.mean()
    y = y - y.mean()
...
def critic_metrics(
    ...
    *,
    ...
    out = {
        **_distribution_metrics("q", q),
        **_distribution_metrics("value", value),
        **_distribution_metrics("advantage", advantage),
    }
    weight_quantiles = torch.quantile(weight.float(), weight.new_tensor([0.5, 0.95, 0.99]))
    absolute_td = td_error.float().abs()
    positive = advantage.float().clamp_min(0.0).mean()
    negative = (-advantage.float()).clamp_min(0.0).mean()
    denominator = (1.0 - tau) * negative
    out.update(
        {
            "actor_weight_mean": weight.float().mean().item(),
            "actor_weight_median": weight_quantiles[0].item(),
            "actor_weight_p95": weight_quantiles[1].item(),
            "actor_weight_p99": weight_quantiles[2].item(),
            "actor_weight_max": weight.float().max().item(),
            "awr_weight_max_frac": weight_clipped.float().mean().item(),
            "td_residual_mean": td_error.float().mean().item(),
            "td_residual_mae": absolute_td.mean().item(),
            "td_residual_rmse": td_error.float().square().mean().sqrt().item(),
            "td_residual_p95": torch.quantile(absolute_td, 0.95).item(),
            "q_target_oob_frac": q_target_oob.float().mean().item(),
            "expectile_balance": float(tau * positive / denominator) if denominator > 0 else float("nan"),
        }
    )
    if returns is not None:
        ...
        out.update(
            {
                "corr_q_g": _correlation(q, target),
                "corr_value_g": _correlation(value, target),
                "q_g_mae": (q.float() - target).abs().mean().item(),
                "value_g_mae": (value.float() - target).abs().mean().item(),
            }
        )
    return out
...
@torch.no_grad()
def val_metrics(model: GPT, batches: list[IQLBatch], cfg: TrainConfig) -> dict[str, object]:
    ...
    q_loss_sum = value_loss_sum = weighted_policy_sum = 0.0
    critic_rows: dict[str, list[Tensor]] = {
        name: []
        for name in ("q", "value", "advantage", "weight", "weight_clipped", "td_error", "q_target_oob", "return")
    }
    try:
        for cpu_iql_batch in batches:
            iql_batch = cpu_iql_batch.to(device)
            batch = iql_batch.batch
            history, targets, valid = prepared_targets(model, batch)
            with amp_context(cfg, device):
                extended_actions = model.codec.quantize(stack_actions(iql_batch.extended_context.features))
                extended_hidden = model(
                    iql_batch.extended_context.features,
                    iql_batch.extended_context.ctx_pad,
                    extended_actions,
                )
                hidden = extended_hidden[:, : cfg.L_ctx]
                next_hidden = extended_hidden[:, EXECUTED_CHUNK : cfg.L_ctx + EXECUTED_CHUNK]
            ...
            q_loss_sum += critic.q_loss.item() * selected_nll.shape[0]
            value_loss_sum += critic.value_loss.item() * selected_nll.shape[0]
            weighted_policy_sum += critic.policy_loss.item() * selected_nll.shape[0]
            for name in ("q", "value", "advantage", "weight", "weight_clipped", "td_error", "q_target_oob"):
                critic_rows[name].append(getattr(critic, name).cpu())
            critic_rows["return"].append(iql_batch.returns[valid].detach().cpu())
            for group, name in enumerate(GROUP_NAMES):
                correct[:, group] += (
    ...
    pooled = {name: torch.cat(rows) for name, rows in critic_rows.items()}
    out["q_loss"] = q_loss_sum / count
    out["value_loss"] = value_loss_sum / count
    out["objective"] = weighted_policy_sum / count / _LN2
    out.update(
        critic_metrics(
            pooled["q"],
            pooled["value"],
...
class BF16Inference:
    ...
    An official evaluation owns one engine for one power-of-two worker bucket.
    Every live and retry wave pads to that bucket, so worker failures cannot create
    a new Dynamo shape. Compilation is synchronous and completes before Dolphin is
    launched; the same model storages continue to be updated by training.
    """
    ...
    def __init__(
        ...
        *,
        ...
        offsets = self.model.head_offsets[:EXECUTED_CHUNK]

        def decoder(hidden, observed, uniforms):
    ...
    def prewarm(self) -> float:
        ...
        self._warmed = True
        print(f"[inference] synchronously compiled batch {self.bucket} in {elapsed:.1f}s", flush=True)
        return elapsed
    ...
    def decode(
        ...
        *,
        ...
        rows = ctx.ctx_pad.shape[0]
        bucket = rows if not self.compiled else self.bucket
        assert bucket is not None
        if rows > bucket:
            raise ValueError(f"inference batch {rows} exceeds fixed compiled bucket {bucket}")
        padded = _pad_context(ctx, bucket)
        padded = canonical_context(padded, self.cfg.observation_bundle)
    ...
    *,
    ...
    def predict(ctx: Context, committed: np.ndarray | None) -> np.ndarray:
        if committed is not None:
            raise ValueError("experiment 027 does not condition on a committed RTC prefix")
        started = time.perf_counter()
...
def _eval_protocol(
    ...
    *,
    ...
    exec_horizon: int,
    checkpoint_sha256: str,
    inference_compile_mode: str = "default",
) -> EvalProtocol:
...
def eval_vs_cpu(
    ...
    *,
    ...
    if inference.compiled and inference.bucket != required_bucket:
        raise ValueError(
            f"evaluation requires compiled bucket {required_bucket}, but the supplied engine owns {inference.bucket}"
        )
    ...
    try:
        # Compile and prove the exact fixed-batch program before Dolphin starts. A
        # compile during the live loop would leave every worker blocked on its first
        # plan request, so fail immediately if a guard was missed.
...
def subsystem_parameter_counts(model: GPT) -> dict[str, int]:
    groups = {
        ...
        "critics": nn.ModuleList([model.q_head, model.value_head]),
    }
...
def model_tag(cfg: TrainConfig) -> str:
    ...
    return (
        f"mtp027-iql4-d{cfg.d_model}-L{cfg.n_layers}-h{cfg.n_heads}-Lc{cfg.L_ctx}-"
        f"t{cfg.temporal_d_model}x{cfg.temporal_layers}-o{offsets}-s{cfg.exec_horizon}-"
        f"tau{cfg.iql_tau:g}-g{cfg.iql_gamma:g}-b{cfg.iql_beta:g}-w{cfg.iql_weight_max:g}-"
...
def loader_kwargs(cfg: TrainConfig, stats: dict[str, FeatureStats]) -> dict:
    ...
    if not v6:
        projection = FeatureProjection(
            columns=BASE_ACTION_PROJECTION.columns | {EGO_REWARD_COLUMN, EGO_RETURN_COLUMN},
...
def validate_batch_geometry(batch: IQLBatch, cfg: TrainConfig, expected_batch_size: int | None = None) -> None:
    ...
    extended_wrong = {
        name: tuple(value.shape)
        for name, value in batch.extended_context.features.items()
        if value.shape[:2] != (batch_size, cfg.L_ctx + EXECUTED_CHUNK)
    }
    if extended_wrong:
        raise ValueError(f"extended context features have the wrong geometry: {extended_wrong}")
    if batch.rewards.shape != (batch_size, cfg.L_ctx, EXECUTED_CHUNK):
        raise ValueError("rewards must be [B, L_ctx, 4]")
    if batch.returns.shape != (batch_size, cfg.L_ctx):
        raise ValueError("returns must be [B, L_ctx]")


def cache_validation(loader: Iterable[IQLBatch], n_samples: int) -> list[IQLBatch]:
    batches: list[IQLBatch] = []
    count = 0
    for batch in loader:
    ...
    for index in range(len(cpu_batches)):
        ...
        for tensor in (
            *ready.batch.context.features.values(),
            ...
            ready.batch.target,
            *ready.extended_context.features.values(),
            ready.extended_context.ctx_pad,
            ready.rewards,
            ready.returns,
        ):
...
def train(
    ...
    *,
    ...
    wandb.init(
        ...
        tags=["gpt", "temporal-mtp", "sparse-offset", "chunk-iql", "027"],
        config=asdict(cfg),
    )
    ...
    inference_engines: dict[int, BF16Inference] = {}

    def evaluation_inference(n_matchups: int) -> BF16Inference:
        bucket = _eval_inference_bucket(cfg, n_matchups)
        engine = inference_engines.get(bucket)
        if engine is None:
            engine = BF16Inference(model, cfg, bucket=bucket)
            inference_engines[bucket] = engine
        return engine
    ...
    try:
        for step in range(start_step, cfg.max_steps):
            ...
            n_prefixes = 0
            q_loss_sum = value_loss_sum = policy_loss_sum = 0.0
            critic_rows: dict[str, list[Tensor]] = {
                name: []
                for name in ("q", "value", "advantage", "weight", "weight_clipped", "td_error", "q_target_oob")
            }
            with profile("step") as stopwatch:
                for iql_batch in device_batches(cpu_batches, DEVICE, copy_stream):
                    batch = iql_batch.batch
                    history, targets, valid = prepared_targets(model, batch)
                    with amp_context(cfg, DEVICE):
                        extended_actions = model.codec.quantize(stack_actions(iql_batch.extended_context.features))
                        extended_hidden = trunk_fn(
                            iql_batch.extended_context.features,
                            iql_batch.extended_context.ctx_pad,
                            extended_actions,
                        )
                        ...
                        loss = (
                            parts.policy_loss
                            + cfg.iql_q_loss_coefficient * parts.q_loss
                            + cfg.iql_value_loss_coefficient * parts.value_loss
                        ) * (n_valid / valid_prefixes)
                    loss.backward()
                    selected_nll = dense_nll[valid]
                    nll_sum += selected_nll.detach().sum(dim=0)
                    n_prefixes += n_valid
                    q_loss_sum += parts.q_loss.item() * n_valid
                    value_loss_sum += parts.value_loss.item() * n_valid
                    policy_loss_sum += parts.policy_loss.item() * n_valid
                    for name in critic_rows:
                        critic_rows[name].append(getattr(parts, name))
            ...
            log = {
                ...
                **{f"train/{name}": value for name, value in metrics.items()},
                "train/objective": policy_loss_sum / n_prefixes / _LN2,
                "train/q_loss": q_loss_sum / n_prefixes,
                "train/value_loss": value_loss_sum / n_prefixes,
                **{f"train/{name}": value for name, value in iql_metrics.items()},
...
def config_from_state(values: dict) -> TrainConfig:
    ...
    if missing:
        raise ValueError(f"checkpoint is not an experiment-027 architecture; missing {sorted(missing)}")
...
def run_benchmark(cfg: TrainConfig, *, iterations: int = 20) -> dict[str, float]:
    ...
    for rows in (1, min(32, ctx.ctx_pad.shape[0])):
        for horizon in (EXECUTED_CHUNK,):
            compiled = BF16Inference(model, cfg, bucket=covering_power_of_two(rows))
