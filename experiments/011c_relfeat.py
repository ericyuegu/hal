"""011c: 011a (compile, AdamW) + explicit opponent-relative spatial features.

Spacing is the core of Melee, but 009's encoding gives only ABSOLUTE per-player
positions, so the model must learn ego↔opp geometry implicitly. A linear input
projection can already form the signed deltas (Δx, Δy are linear in the inputs),
so those add nothing — the value is in the NONLINEAR relations: a pseudo-distance,
|Δx|/|Δy|, and mutual facing (does ego face opp / opp face ego). We compute these
per frame in-model from the already-standardized ego/opp position + direction
columns (ego & opp share one normalization key, so standardized deltas are the
true deltas up to a constant scale) and concat them onto every context token.
Everything else is identical to 011a (compile, AdamW, b128).
