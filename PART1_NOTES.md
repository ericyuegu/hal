...
`position` argument; `max_cache` now lives on the trunk and is `attn_window or L_ctx`). One call-site
detail: the trunk returns `[B, L, d_model]`, so 022 keeps 020's `[:, -1]` at the caller. 020's
one-token guard (`020:863-864`) now lives in the trunk, which raises for `L != 1` — the cached
attention applies no mask, so a chunk would let its tokens see each other.
