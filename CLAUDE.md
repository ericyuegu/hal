...
- Three "scripts" homes, by tier — don't conflate: `hal/scripts/` is package code (data-pipeline stage CLIs, shipped in the wheel, run as `python -m hal.scripts.X`); root `scripts/` is host ops that does NOT import `hal` (dev-box `setup.sh`, the `launch_vast.py` / `launch_gce.py` cloud launchers); `docker/` is the cloud image and what runs *inside* it (`Dockerfile`, `compose.yaml`, `entrypoint.sh`, and the per-provider boot scripts `on-start.sh` for vast, `on-start-gce.sh` for GCE). The launchers live in `scripts/`, not `docker/`, because they run on the host and are never baked into the image.
