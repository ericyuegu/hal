...
def _resolve_export_stats(checkpoint_config: Mapping[str, object]) -> _ResolvedStats:
    ...
    sources_by_name = {source.name: source for source in streams.POLICY_WORLD_V8_SOURCES}
    paths = tuple(streams.ensure_stats(sources_by_name[name].local_root / "stats.json") for name in source_names)
