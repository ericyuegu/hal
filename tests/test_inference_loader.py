...
from hal.inference.backends.history_decoder import policy as history_decoder
from hal.inference.backends.temporal_awr import policy as temporal_awr


def test_o59_defaults_to_kv_cache_and_allows_window_control(monkeypatch: pytest.MonkeyPatch) -> None:
    manifest = SimpleNamespace(
        backend=history_decoder.O59_BACKEND, backend_version=history_decoder.O59_BACKEND_VERSION
    )
    ...
    monkeypatch.setattr(history_decoder, "load_o59_policy", load)
...
def test_o50_keeps_window_default_and_rejects_kv_cache(monkeypatch: pytest.MonkeyPatch) -> None:
    manifest = SimpleNamespace(backend=temporal_awr.O50_BACKEND, backend_version=temporal_awr.O50_BACKEND_VERSION)
    monkeypatch.setattr(loader, "read_policy_manifest", lambda _path: manifest)
    loaded: list[object] = []
    monkeypatch.setattr(temporal_awr, "load_o50_policy", lambda *_args, **_options: loaded.append(object()))
    loader.load_policy("policy.hal")
    assert len(loaded) == 1
...
def test_history_mode_rejects_invalid_value() -> None:
    with pytest.raises(ValueError, match="unsupported history mode"):
        loader.resolve_history_mode(temporal_awr.O50_BACKEND, "invalid")  # type: ignore[arg-type]
